...
export function applyVisionHelper(ctx: PluginContext, config: Config): void {
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    name: 'generate_image',
    description:
      '调用已配置的生图模型生成一张或多张图片。当用户要求生成、绘制、创建图片或图像时使用本工具，提示词越详细越好；用户要求"几张/多张"时用 count 指定张数（最多 4 张），一次调用返回全部图片。若返回 ok=false，请把 error 信息转告用户（生图模型在「设置 → AI 模型」中配置）。',
    parameters: {
      prompt: {
        type: 'string',
        required: true,
    ...
    async execute(args: { prompt: string; count?: number }, exec: any): Promise<any> {
  ...
  async function saveVisionList(list: Array<{ provider: string; model: string }>, active?: string): Promise<void> {
    const target = await ctx.fs.resolve(config.modelRouterPath || '.dsh/model-router.json')
    let parsed: any = {}
    try { parsed = JSON.parse(await ctx.fs.readText(target)) } catch { /* 无文件则新建 */ }
