 ...
 */
export function alignTextDiff(oldText: string, newText: string): AlignResult {
  ...
  const pushCtx = (oi: number, ni: number): void => {
    rows.push({
      kind: 'ctx',
  ...
  const pushMod = (oi: number, ni: number): void => {
    rows.push({
      kind: 'mod',
      l: { no: oi + 1, text: oldLines[oi]! },
      r: { no: ni + 1, text: newLines[ni]! },
  ...
  const pushDel = (oi: number): void => {
    rows.push({ kind: 'del', l: { no: oi + 1, text: oldLines[oi]! } })
    removed += 1
  }
  const pushAdd = (ni: number): void => {
  ...
  for (let i = 0; i < prefix; i++) pushCtx(i, i)
  ...
  if (n > 0 && m > 0 && (n + 1) * (m + 1) <= MAX_DP_CELLS) {
    ...
    while (i < n) { ops.push({ k: 'del', oi: i }); i += 1 }
    while (j < m) { ops.push({ k: 'add', ni: j }); j += 1 }
  } else if (n > 0 || m > 0) {
    // 超预算降级：按行号顺序两两配成 mod，剩余一侧单独成块。
    const paired = Math.min(n, m)
    for (let k = 0; k < paired; k++) ops.push({ k: 'pair', oi: k, ni: k })
    for (let k = paired; k < n; k++) ops.push({ k: 'del', oi: k })
    for (let k = paired; k < m; k++) ops.push({ k: 'add', ni: k })
  }
  ...
  while (idx < ops.length) {
    ...
    const dels: number[] = []
    while (idx < ops.length && ops[idx]!.k === 'del') { dels.push(ops[idx]!.oi!); idx += 1 }
    const adds: number[] = []
    while (idx < ops.length && ops[idx]!.k === 'add') { adds.push(ops[idx]!.ni!); idx += 1 }
    const pairs = Math.min(dels.length, adds.length)
    for (let p = 0; p < pairs; p++) pushMod(oldStart + dels[p]!, newStart + adds[p]!)
    for (let p = pairs; p < dels.length; p++) pushDel(oldStart + dels[p]!)
    for (let p = pairs; p < adds.length; p++) pushAdd(newStart + adds[p]!)
  }

  for (let s = 0; s < suffix; s++) pushCtx(oldEnd + s, newEnd + s)
