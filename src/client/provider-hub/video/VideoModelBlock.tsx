 ...
 * 候选模型标注「（生视频）」——声明了 capabilities 含 video 的模型
 * （在供应商的模型设置中勾选「支持生视频」）。
 */
...
function isVideoModel(m: ModelInfo): boolean {
  return Array.isArray(m.outputs) && m.outputs.includes('video')
}

export function VideoModelBlock(): React.ReactElement {
          ...
          <>
            <div style={ROW}>
              ...
              >
                ...
                {currentModels.map(m => (
                  <option key={m.id} value={m.id}>{m.name || m.id}{isVideoModel(m) ? '（生视频）' : ''}</option>
                ))}
              </select>
            </div>
