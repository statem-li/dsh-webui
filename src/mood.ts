...
const MOOD_CONTRACT = [
  '【MOOD 自述 · 必须最先输出】每一轮里，你的可见输出必须以一个 ```mood 代码围栏开头：',
  '思考一结束就先写完这个围栏，再开始任何正文、解释或工具调用——',
  'MOOD 永远排在本轮回答的最前面，绝不放到结尾补写。',
  '它写的是你此刻（刚想完、尚未动笔）的状态；小节带回顾口吻也没关系，',
  '那是对思路的预演，不影响它最先出现。整个回合只在第一次开口时写一次，',
  '后续工具调用之间的过渡文字不再重复。',
  '围栏内格式：每个小节单独一行「小节名:」，其下用「- 」列条目；每节最多 3 条，每条一句话。',
  '围栏里不写代码、不写结论清单、不写对用户的答复——那些都属于围栏之后的正式回答。',
...
function normalizeEntry(input: unknown, base?: MoodEntry): MoodEntry {
...
function buildInstruction(persona: string): string {
 ...
 * @param ctx - host 上下文。
 */
export function applyMood(ctx: PluginContext): void {
  ...
  const readConfig = (): MoodConfig => {
    if (scope !== undefined) {
      try {
        const value = scope.get() as Partial<MoodConfig> | undefined
        const presets: Record<string, MoodEntry> = {}
  ...
  const presetIdOf = (context: any): string => {
    ...
    try { return presets.composedPreset(agentCtx) ?? '' } catch { return '' }
  }

  const render = (context: any): string => {
  ...
  ctx.effect(() => ctx.systemPrompt.section({
    ...
    order: -20,
    text: (context: any) => render(context),
  }), 'webui: mood prompt section')

  // ── HTTP API ──────────────────────────────────────────────────────────────

  /** 当前 preset 名单（设置页据此列行；服务缺失时返回空数组）。 */
  const readRoster = async (): Promise<Array<Record<string, unknown>>> => {
    const presets = ctx.get?.('agentPresets')
  ...
  const applyPatch = async (body: any): Promise<void> => {
    if (scope === undefined || body === null || typeof body !== 'object') return
    const current = readConfig()
    const next: MoodConfig = {
    ...
    if (body.presets !== null && typeof body.presets === 'object') {
      for (const [id, patch] of Object.entries(body.presets as Record<string, unknown>)) {
        if (!isPresetId(id)) continue
        // 显式 null = 删除该 Agent 的覆盖，回落到默认人设。
        if (patch === null) { delete next.presets[id]; continue }
        if (next.presets[id] === undefined && Object.keys(next.presets).length >= MAX_PRESET_ENTRIES) continue
        next.presets[id] = normalizeEntry(patch, next.presets[id])
      }
    }
    await scope.update(MoodSchema(next as never))
  }
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req: any, res: any) => {
      ...
      try {
        if (req.method === 'POST') await applyPatch(await readBody(req))
        const config = readConfig()
        respond(200, {
          ok: true,
