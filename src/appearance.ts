...
const clampOpacity = (v: unknown): number => {
  const n = Math.round(Number(v))
  if (!Number.isFinite(n)) return APPEARANCE_DEFAULT_OPACITY
 ...
 * @param ctx - host 上下文。
 */
export function applyAppearance(ctx: PluginContext): void {
  ...
  const readGlass = (): boolean => {
    if (scope !== undefined) {
      try { return scope.get().glass === true } catch { /* fallthrough */ }
    }
  ...
  const readOpacity = (): number => {
    if (scope !== undefined) {
      try { return clampOpacity(scope.get().opacity) } catch { /* fallthrough */ }
    }
