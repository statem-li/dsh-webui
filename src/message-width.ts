...
export const MESSAGE_WIDTH_DEFAULT = { value: 525, unit: 'px' } as const

/** 单位各自的有效范围：百分比 10–100，像素 120–1600。 */
const RANGES: Record<MessageWidthUnit, { min: number; max: number }> = {
  '%': { min: 10, max: 100 },
  px: { min: 120, max: 1600 },
}
...
function normalize(value: unknown, unit: unknown): { value: number; unit: MessageWidthUnit } | null {
  const u: MessageWidthUnit = unit === '%' ? '%' : 'px'
...
export function applyMessageWidth(ctx: PluginContext): void {
  ...
  const read = (): { value: number; unit: MessageWidthUnit } => {
    if (scope !== undefined) {
      try {
        const s = scope.get()
    ...
    handler: async (req: any, res: any) => {
      try {
        if (req.method === 'POST' && scope !== undefined) {
          ...
          if (next !== null) await scope.update({ value: next.value, unit: next.unit })
        }
