...
/** 一项指标的统计摘要。 */
export interface BenchSummary { avg: number; p50: number; p95: number; min: number; max: number }
...
const PREFILL_UNIT =
  '大语言模型的推理延迟由预填充与解码两个阶段构成：预填充并行处理整段输入并构建 KV 缓存，'
...
function newStage(key: string, name: string, unit: string, note: string): BenchStage {
  return { key, name, unit, status: 'pending', note, samples: [], estimated: false }
}

function summarize(samples: number[]): BenchSummary {
  const s = [...samples].sort((a, b) => a - b)
  const pick = (q: number): number => s.length === 0 ? 0 : s[Math.min(s.length - 1, Math.floor(q * s.length))]!
 ...
 */
async function measureWithRetry<T>(attempt: () => Promise<T>): Promise<T> {
  const maxRetries = 2
  let retried = 0
  for (;;) {
    try {
      ...
      if (!transient || retried >= maxRetries) {
        if (retried > 0) throw new Error(message + '｜已自动重试 ' + retried + ' 次仍失败')
        throw error
      }
...
async function streamOnce(
  ...
  try {
    ...
    while (!doneReading) {
      ...
      if (rawHead.length < 240) rawHead += chunkText
      buffer += chunkText
...
export function benchSnapshot(): BenchSnapshot | null {
  if (bench === null) return null
  const summaries: Record<string, BenchSummary> = {}
  for (const stage of bench.stages) {
    if (stage.samples.length > 0) summaries[stage.key] = summarize(stage.samples)
  }
  const end = bench.finishedAt ?? Date.now()
  return { ...bench, elapsedMs: end - bench.startedAt, summaries }
}

function stage(benchState: BenchState, key: string): BenchStage {
  return benchState.stages.find(s => s.key === key)!
}

/** 是否还有预算进入下一阶段（留 12s 余量收尾）。 */
function budgetLeft(benchState: BenchState): boolean {
  return Date.now() - benchState.startedAt < BUDGET_MS - 12_000
}

async function runBench(ctx: PluginContext, provider: string, model: string): Promise<void> {
  const state = bench!
  try {
    ...
    try {
      ...
      if (/400/.test(String(error?.message ?? ''))) {
        withUsage = false
        for (const s of state.stages) s.estimated = true
      } else {
        throw error
      }
    }
    ...
    if (budgetLeft(state)) {
      ...
      try {
        for (let i = 0; i < 3 && budgetLeft(state); i++) {
          ...
          if (r.estimated) tpsStage.estimated = true
        }
    ...
    if (budgetLeft(state)) {
      ...
      try {
        ...
        if (lastPromptTokens > 0) prefillStage.note += `｜实际输入 ${lastPromptTokens} tokens`
    ...
    } else prefillStage.status = 'skipped'
...
/** 启动一次基准测试（单例；已在跑则拒绝）。 */
export function startBench(ctx: PluginContext, provider: string, model: string): { ok: boolean; error?: string } {
...
function detectSnapshot(): DetectState | null {
  return detect === null ? null : { ...detect, items: detect.items.map(i => ({ ...i })) }
}

async function runDetectAsync(ctx: PluginContext, provider: string, model: string): Promise<void> {
  const st = detect!
  const itemOf = (key: string): DetectItem => st.items.find(i => i.key === key)!
  const finish = (): void => {
    st.running = false
    st.finishedAt = Date.now()
    for (const it of st.items) if (it.status !== 'done') { it.status = 'done'; if (it.ok === null && it.note === '') it.note = '未检测（预算耗尽）' }
  }
  try {
    const profile = providerConfig(ctx, provider)
    if (!profile || typeof profile.baseURL !== 'string' || !profile.baseURL) {
    ...
    for (const level of DETECT_LEVELS) {
      ...
      if (/anthropic/i.test(apiKind)) { li.ok = null; li.note = 'anthropic 协议暂不支持自动探测'; li.status = 'done'; continue }
      if (Date.now() > deadline) { li.ok = null; li.note = '预算耗尽，未探测'; li.status = 'done'; continue }
      li.status = 'running'
      let supported = false
      try {
        const ctrl = new AbortController()
        ...
        if (level !== 'off') payload.reasoning_effort = level
        try {
      ...
      if (provSection && Array.isArray(provSection.models)) {
        ...
        if (idx >= 0) {
          ...
          if (thinkers.length > 0) {
            ...
            for (const l of thinkers) efforts[l] = l
            nextModels[idx]!.reasoningEfforts = efforts
            st.savedLevels = true
          }
          await (ctx.settings as any).update(ns, { providers: { [provider]: { models: nextModels } } })
        }
      }
    } catch (error: any) {
...
/** 注册 HTTP 接口。 */
export function applyPerfBench(ctx: PluginContext): void {
  ctx.effect(() => {
    ...
    return webServer.register({
      ...
      handler: async (req: any, res: any) => {
        ...
        try {
          if (req.method === 'POST') {
            const body: any = await new Promise<any>((resolve) => {
              let data = ''
            ...
            if (!provider || !model) return reply(400, { ok: false, error: 'provider/model 不能为空' })
        ...
        try {
          if (req.method === 'POST') {
            ...
            await new Promise<void>((resolve) => {
              let data = ''
              req.on('data', (chunk: any) => { data += chunk })
              req.on('end', () => { try { body = JSON.parse(data || '{}') } catch { body = {} } resolve() })
              req.on('error', () => resolve())
            })
            const provider = typeof body.provider === 'string' ? body.provider : ''
            const model = typeof body.model === 'string' ? body.model : ''
