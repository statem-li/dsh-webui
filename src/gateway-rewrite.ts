...
interface GatewayRewriteConfig {
  enabled: boolean
  rules: RewriteRule[]
}
...
let rewriteState: GatewayRewriteConfig | null = null
...
function agentFor(proxyUrl: string): any {
...
function normalizeRules(input: readonly unknown[]): RewriteRule[] {
  ...
  for (const raw of input) {
    ...
    if (host === '' || userAgent === '') continue
    if (proxyUrl !== '' && !/^https?:\/\/.+/.test(proxyUrl)) continue
...
function matchRule(host: string, rules: readonly RewriteRule[]): RewriteRule | null {
...
function installHook(): void {
  ...
  globalThis.fetch = function (input: any, init?: any) {
    ...
    const next: any = init === undefined || init === null ? {} : { ...init }
    ...
    if (rule.proxyUrl !== '') {
      ...
      if (agent !== undefined) next.dispatcher = agent
    }
    return original(href ?? input, next)
  }
}

export function applyGatewayRewrite(ctx: PluginContext): void {
  ...
  const readConfig = (): GatewayRewriteConfig => {
    if (scope !== undefined) {
      try {
        const v = scope.get()
  ...
  const applyConfig = (cfg: GatewayRewriteConfig): void => {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req: any, res: any) => {
      try {
        ...
        const next: GatewayRewriteConfig = { enabled, rules }
        applyConfig(next)
