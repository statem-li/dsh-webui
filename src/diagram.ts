...
function readVersion(): string {
  try {
    if (existsSync(VERSION_FILE)) return readFileSync(VERSION_FILE, 'utf8').trim() || 'unknown'
  } catch { /* 忽略 */ }
  return 'unknown'
}

/** 注册图表资源路由 + 作图提示词开关。 */
export function applyDiagram(ctx: PluginContext): void {
  ...
  const load = (): Buffer | null => {
    ...
    try {
      if (!existsSync(MERMAID_GZ) || statSync(MERMAID_GZ).size === 0) return null
  ...
  const hintEnabled = (): boolean => {
    if (scope !== undefined) {
      try { return scope.get().promptHint !== false } catch { /* fallthrough */ }
    }
    return true
  }
