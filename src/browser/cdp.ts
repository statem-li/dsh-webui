...
export async function fetchBrowserWsUrl(port: number, timeoutMs = 5000): Promise<string> {
...
/** 列出所有 page target */
export async function listPageTargets(conn: CdpConnection): Promise<CdpTarget[]> {
  const res: any = await conn.send('Target.getTargets')
  return (res?.targetInfos || []).filter((t: any) => t.type === 'page')
}
...
export async function createPageSession(
  ...
  const created: any = await conn.send('Target.createTarget', { url })
  const targetId: string = created.targetId
  return attachTarget(conn, targetId)
}

/** attach 已有 target */
export async function attachTarget(conn: CdpConnection, targetId: string): Promise<CdpSession> {
  const attached: any = await conn.send('Target.attachToTarget', { targetId, flatten: true })
...
export async function navigateHistory(session: CdpSession, delta: number): Promise<{ url: string; title: string }> {
  const { conn, sessionId } = session
  const info: any = await conn.send('Page.getNavigationHistory', {}, sessionId)
  const entries: any[] = info?.entries || []
  const idx: number = info?.currentIndex ?? -1
  ...
  const after: any = await conn.send('Page.getNavigationHistory', {}, sessionId)
...
export async function clearViewport(session: CdpSession): Promise<void> {
  const { conn, sessionId } = session
...
/** 页面执行 JS，返回 JSON 值 */
export async function evaluateJson(
  session: CdpSession,
  expression: string,
  awaitPromise = true,
): Promise<any> {
...
export async function getViewportSize(session: CdpSession): Promise<{ width: number; height: number }> {
  const v = await evaluateJson(
    session,
...
export async function inspectElementAt(session: CdpSession, x: number, y: number): Promise<InspectedElement> {
  ...
  const empty: InspectedElement = { found: false, selector: '', tag: '', id: '', className: '', role: '', text: '', label: '', rect: null, vw: 0, vh: 0 }
...
export async function dispatchMouseMove(session: CdpSession, x: number, y: number): Promise<void> {
  const { conn, sessionId } = session
  await conn.send('Input.dispatchMouseEvent', { type: 'mouseMoved', x, y, button: 'none' }, sessionId)
}
...
export async function dispatchEnterKey(session: CdpSession): Promise<void> {
  await dispatchKey(session, 'Enter')
}

// 修饰键位掩码（CDP modifiers）：Alt=1 Ctrl=2 Meta=4 Shift=8
const MOD_BITS: Record<string, number> = { alt: 1, ctrl: 2, control: 2, meta: 4, cmd: 4, shift: 8 }

interface KeyDef { key: string; code: string; vk: number; text?: string }
