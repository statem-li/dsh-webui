...
export function applyBrowser(ctx: PluginContext, config: Config): void {
  ...
  const tools = [
    ...
    defineTool({
      ...
      description: '在浏览器打开 URL 并等待加载（load + 网络空闲），返回页面 ref 树。能构造出最终地址（含搜索结果页/详情页 URL）时直接打开目标页，避免「首页→搜索→点结果」式逐跳导航。',
      parameters: {
      ...
      async execute(args: { url: string }, exec: any): Promise<any> {
        ...
        try {
          ...
          step.label = `打开 ${(() => { try { return new URL(info.url).hostname } catch { return info.url } })()}`
          const snap = await snapshotFor(session, sessionId)
    ...
    defineTool({
      name: 'browser_click',
      description: '点击页面元素（ref 来自最新 snapshot），返回操作后最新 snapshot。连续多步操作请改用 browser_batch 合并或 browser_evaluate 一段 JS 批量完成；单步调用仅用于需观察中间结果再决定下一步、或一次性独立点击。已知不变的页面可设 returnSnapshot=false 跳过快照以提速。',
      parameters: {
    ...
    defineTool({
      name: 'browser_type',
      description: '向输入框输入文本（ref 来自最新 snapshot）。对下拉框 select 也会按文本/值选择。返回操作后最新 snapshot；可设 returnSnapshot=false 跳过。要填多个字段时优先用 browser_batch 合并或 browser_evaluate 一段 JS 批量完成，不要逐框调用本工具。',
      parameters: {
        ref: { type: 'number', required: true, description: 'snapshot 中的 [ref] 编号' },
    ...
    defineTool({
      ...
      output: { schema: { type: 'json' }, render: (_a: unknown, v: unknown) => [{ type: 'text', text: JSON.stringify(v, null, 2) }] },
      async execute(args: { actions: any[] }, exec: any): Promise<any> {
        const sessionId = sessionIdOf(exec)
        const actions = Array.isArray(args.actions) ? args.actions : []
        if (actions.length === 0) return { ok: false, error: 'actions 不能为空' }
        if (actions.length > 10) return { ok: false, error: '单次最多 10 个动作' }
          ...
          for (let i = 0; i < actions.length; i++) {
            ...
            switch (kind) {
              ...
              case 'scroll': {
                ...
                await scrollPage(session, dir as any, Number(a.amount) || 3)
              ...
              case 'navigate': {
                ...
                const info = await navigateAndWait(session, url, NAV_TIMEOUT_MS)
                labels.push(`打开 ${(() => { try { return new URL(info.url).hostname } catch { return url } })()}`)
                break
              }
              default:
    ...
    defineTool({
      ...
      description: '在页面执行 JavaScript 表达式并返回结果（JSON 序列化）。用于处理 ref 树定位不到的元素（弹窗、iframe、自定义控件）。批量填表、批量勾选/下拉、读取或核对页面数据等场景优先用本工具一段 JS 一次完成（React 受控输入需经 native setter 赋值并 dispatch input/change 事件），比逐个 click/type 快一个数量级。',
      parameters: {
          ...
          if (typeof describeFn === 'function') {
            try {
              ...
              const res = await Promise.race([
                ...
                new Promise<never>((_, rej) => setTimeout(() => rej(new Error('视觉描述超时（60s）')), 60000)),
              ])
