...
interface SessionBrowserState {
  ...
  offFrame: (() => void) | null
  /** 原生弹窗守卫（Page.javascriptDialogOpening）的取消函数。 */
  offDialog: (() => void) | null
  /** 最近一次操作后是否已确认页面稳定（用于跳过重复的 settle 等待）。 */
  lastSettleAt: number
  /** Chrome 窗口外框与页面视口的高度差（标题栏等 UI），首次贴合时测量并缓存。 */
  chromeUiHeight: number | null
  /** true = 壳内嵌视图模式（WebContentsView，零独立窗口）；false = 独立 Edge/Chrome 兜底。 */
  shellMode: boolean
  /** 壳内视图的会话键（sanitize 后的 sessionId）。 */
  bvKey: string
  /** 壳内多标签：tabId → CDP targetId（target 生命周期=视图生命周期）。 */
...
export function applyBrowser(ctx: PluginContext, config: Config): void {
    ...
    try {
      const files = fs.readdirSync(dir)
        ...
        .map(n => {
          ...
          let mtime = 0
          try { mtime = fs.statSync(full).mtimeMs } catch { /* 已被删 */ }
          return { full, mtime }
        })
        .sort((a, b) => b.mtime - a.mtime)
      for (const f of files.slice(MAX_SHOTS_PER_SESSION)) {
        try { fs.unlinkSync(f.full) } catch { /* 占用中则下次再删 */ }
      }
    } catch { /* 目录不存在等：忽略 */ }
  }

  function screenshotDirFor(sessionId: string): string {
    ...
    try { fs.mkdirSync(dir, { recursive: true }) } catch { /* 创建失败时写文件会报错，交由调用方感知 */ }
    return dir
  }
  ...
  // 会话级「禁止」集合（仅内存，不持久化）：对话面板卡片上的「禁止」只拦当前
  // 会话，不写全局 allowBrowser；服务重启即清空（会话粒度的临时开关）。
  ...
  async function bvPost(pathname: string, body: Record<string, unknown>, timeoutMs = 1500): Promise<any> {
  ...
  function attachDialogGuard(sessionId: string, conn: CdpConnection): () => void {
    return conn.on('Page.javascriptDialogOpening', (p: any) => {
  ...
  function posWord(x: number, y: number, vw: number, vh: number): string {
    if (vw <= 0 || vh <= 0) return ''
  ...
  function tagCn(tag: string): string {
    switch (tag) {
  ...
  function targetWord(tag: string, text: string): string {
    const t = text.trim()
    if (t !== '') return `「${t.length > 16 ? t.slice(0, 16) + '…' : t}」`
    return tagCn(tag)
  }
  function shortText(s: string, n = 20): string {
  ...
  function scrollWord(dir: string): string {
    return dir === 'up' ? '向上' : dir === 'down' ? '向下' : dir === 'left' ? '向左' : '向右'
  }
  /** 截图文件的对外可访问 URL（统一拼装，避免各处手写 query 拼错）。 */
  function shotUrl(sessionId: string, file: string): string {
  ...
  function wrapExpression(expr: string): string {
    ...
    for (let i = 0; i < src.length; i++) {
      ...
      if (c === ')' || c === ']' || c === '}') { depth--; continue }
      if (depth !== 0) continue
      if ((c === ';' || c === '\n') && src.slice(i + 1).trim() !== '') topSeparator = true
      if (c === 'r' && src.startsWith('return', i) && isTokenBoundary(src, i, 6)) topReturn = true
      if (c === 'a' && src.startsWith('await', i) && isTokenBoundary(src, i, 5)) hasAwait = true
    }
    if (!topReturn && !topSeparator && !hasAwait) return src
    const body = topReturn || topSeparator ? src : `return (${src})`
    return `(async () => { ${body} })()`
  }

  /** 关键字前后都不是标识符字符（避免匹配 returnValue / myawait）。 */
  function isTokenBoundary(src: string, at: number, len: number): boolean {
  ...
  // ═══ 浏览器工具门禁：全局关闭拦全部；会话级 deny 只拦该会话 ═══
  ctx.effect(() => ctx.on('tools/pre-execute', async (exec: any, next: any) => {
    if (typeof exec?.name === 'string' && exec.name.startsWith('browser_')) {
      if (!allowBrowser) {
        return { kind: 'deny', reason: '浏览器使用已被用户禁用（可在对话面板开关中开启）' }
      }
      if (sessionDeny.has(sessionIdOf(exec))) {
        return { kind: 'deny', reason: '浏览器使用已被用户在本会话禁用（可悬停浏览器按钮重新允许）' }
      }
    }
    return next()
  ...
  async function syncActiveTabSession(st: SessionBrowserState): Promise<void> {
    ...
    if (!st.session || st.session.targetId !== targetId) {
      ...
      try { await st.conn.send('Page.enable', {}, st.session.sessionId) } catch { /* 已启用 */ }
    }
  }
  ...
  async function rebindShellTabs(st: SessionBrowserState): Promise<boolean> {
      ...
      for (const p of pages) {
        try {
          ...
          if (typeof name === 'string' && name.startsWith(prefix)) {
            ...
            if (tabId !== '') fresh.set(tabId, p.targetId)
          }
  ...
  async function waitShellTarget(conn: CdpConnection, bvKey: string, tabId: string): Promise<string | null> {
    ...
    while (Date.now() < deadline) {
      ...
      if (wait < 120) wait = Math.min(120, wait * 2)
    }
    return null
  }

  /** 壳内新建标签页（可选直接导航），并设为激活标签。 */
  async function shellNewTab(sessionId: string, st: SessionBrowserState, navigateUrl?: string): Promise<string> {
    const conn = st.conn
  ...
  const tabsCache = new Map<string, { at: number; tabs: any[] }>()

  /** 壳内标签列表（title/url 来自 CDP target 信息）。 */
  async function shellTabsInfo(st: SessionBrowserState): Promise<any[]> {
    if (!st.conn?.connected || st.tabTargets.size === 0) return []
    const cached = tabsCache.get(st.bvKey)
    if (cached && Date.now() - cached.at < 600) return cached.tabs
    let pages: any[] = []
    try {
    ...
    const byId = new Map<string, any>(pages.map((p) => [p.targetId, p]))
    const out: any[] = []
  ...
  async function startBrowserFor(sessionId: string): Promise<any> {
    ...
    try {
      ...
      if (st.offDialog) { try { st.offDialog() } catch { /* 已解绑 */ } }
      ...
      try { await evaluateJson(session, `window.name = ${JSON.stringify(`dshbv-${st.bvKey}/${tabId}`)}`, false) } catch { /* 不致命 */ }
      st.session = session
  ...
  async function stopBrowserFor(sessionId: string): Promise<any> {
    ...
    if (st.offDialog) { try { st.offDialog() } catch {} }
    st.offDialog = null
    st.frame = null
    // 壳内视图模式：销毁壳里的 WebContentsView（无进程可杀）
    if (st.shellMode) {
      try { await bvPost('/view/close', { sessionKey: st.bvKey }, 2000) } catch { /* 壳不在了 */ }
   ...
   */
  function ensureFrameStream(st: SessionBrowserState): void {
    if (st.offFrame !== null || st.conn === null) return
    const conn = st.conn
    st.offFrame = conn.on('Page.screencastFrame', (p: any) => {
      const target = st.session
      if (target === null) return
      if (p?.sessionId !== undefined && p.sessionId !== target.sessionId) return
  ...
  /** 解除帧订阅（抽屉关闭 / 停止浏览器）。 */
  function releaseFrameStream(st: SessionBrowserState): void {
    if (st.offFrame !== null) { try { st.offFrame() } catch { /* 已解绑 */ } }
    st.offFrame = null
  }
  ...
  function locatorOf(args: any): Locator {
    return {
  ...
  function requireLocator(loc: Locator): Locator {
  ...
  async function runTool<T>(
    ...
    label: string,
    detail: string,
    fn: (args: { sessionId: string; step: ActivityStep }) => Promise<T>,
  ...
  const LOCATOR_PARAMS = {
    ref: { type: 'number' as const, description: 'snapshot 中的 [ref] 编号（最快）' },
    selector: { type: 'string' as const, description: 'CSS 选择器（自动穿透 shadow DOM / 同源 iframe）；快照失效时用它免去重拍' },
    text_match: { type: 'string' as const, description: '元素可见文本/aria-label/placeholder 匹配串（精确优先，其次前缀、包含）' },
    nth: { type: 'number' as const, description: 'selector/text_match 命中多个时取第几个（1 起，默认 1）' },
  }
  ...
  const tools = [
    ...
    defineTool({
      ...
      async execute(args: any, exec: any): Promise<any> {
        return runTool(exec, 'browser_navigate', '打开网页', '', async ({ sessionId, step }) => {
          ...
          if (url === null) throw new Error('url 无效（仅支持 http/https）')
          ...
          if (args?.returnSnapshot === false) return { ok: true, url: info.url, title: info.title, ...(waited ? { waited } : {}) }
          const snap = await snapshotFor(session, sessionId)
    ...
    defineTool({
      ...
      async execute(args: any, exec: any): Promise<any> {
        return runTool(exec, 'browser_click', '点击页面元素', '', async ({ sessionId, step }) => {
          ...
          if (args?.returnSnapshot === false) return { ok: true, clicked: step.label }
    ...
    defineTool({
      ...
      async execute(args: any, exec: any): Promise<any> {
        return runTool(exec, 'browser_press', '按键', '', async ({ sessionId, step }) => {
          ...
          for (let i = 0; i < repeat; i++) await dispatchKey(session, key, mods)
          step.label = `按 ${mods.map((m: string) => `${m}+`).join('')}${key}${repeat > 1 ? ` ×${repeat}` : ''}`
    ...
    defineTool({
      ...
      async execute(args: any, exec: any): Promise<any> {
        ...
        if (actions.length === 0) return { ok: false, error: 'actions 不能为空' }
        if (actions.length > MAX_BATCH_ACTIONS) return { ok: false, error: `单次最多 ${MAX_BATCH_ACTIONS} 个动作` }
        const labels: string[] = []
        const outcome: any = await runTool(exec, 'browser_batch', `批量执行 ${actions.length} 个动作`, '', async ({ sessionId, step }) => {
          const session = await requireSession(sessionId)
          for (let i = 0; i < actions.length; i++) {
            const a = (actions[i] ?? {}) as Record<string, any>
            ...
            switch (kind) {
              ...
              case 'press': {
                ...
                for (let k = 0; k < repeat; k++) await dispatchKey(session, String(a.key ?? ''), mods)
                labels.push(`按 ${mods.map((m: string) => m + '+').join('')}${a.key}${repeat > 1 ? ` ×${repeat}` : ''}`)
                break
              }
              case 'scroll': {
                const dir = ['up', 'down', 'left', 'right'].includes(String(a.direction)) ? String(a.direction) : 'down'
                await scrollPage(session, dir as any, Number(a.amount) || 3, typeof a.selector === 'string' ? a.selector : undefined)
              ...
              case 'wait': {
                ...
                if (!w.ok) throw new Error(w.error ?? '等待条件未满足')
          ...
          if (args?.returnSnapshot === false) return { ok: true, actions: labels }
    ...
    defineTool({
      ...
      async execute(args: any, exec: any): Promise<any> {
        return runTool(exec, 'browser_wait_for', '等待页面条件', '', async ({ sessionId, step }) => {
          ...
          if (sel === '' && txt === '') throw new Error('需要 selector 或 text 之一')
          const session = await requireSession(sessionId)
    ...
    defineTool({
      ...
      async execute(args: any, exec: any): Promise<any> {
        return runTool(exec, 'browser_scroll', '滚动页面', '', async ({ sessionId, step }) => {
          ...
          if (args?.returnSnapshot === false) return { ok: true, ...res }
          ...
          if (typeof describeFn === 'function') {
            try {
              ...
              const res = await Promise.race([
                ...
                new Promise<never>((_, rej) => setTimeout(() => rej(new Error('视觉描述超时（60s）')), VISION_TIMEOUT_MS)),
              ])
  ...
  function readJsonBody(req: any): Promise<any> {
    return new Promise((resolve) => {
      ...
      req.on('data', (chunk: any) => {
        if (tooLarge) return
        raw += chunk
        if (raw.length > 1_000_000) { tooLarge = true; raw = '' }
      })
      req.on('end', () => {
  ...
  /** 运行中的会话态（conn/session 已窄化为非空，路由内无需再断言）。 */
  interface LiveState {
    st: SessionBrowserState
    conn: CdpConnection
    session: CdpSession
  }

  /** 路由通用：取运行中的会话态；未运行时已应答 404/400 并返回 null。 */
  function runningState(res: any, sessionId: unknown): LiveState | null {
  ...
  function isRunning(st: SessionBrowserState | undefined): boolean {
    if (!st) return false
    return st.shellMode
    ...
    return webServer.register({
      ...
      handler: async (req: any, res: any) => {
        try {
          ...
          let tabs: any[] = []
          if (st?.shellMode) {
            try { tabs = await shellTabsInfo(st) } catch { /* 忽略 */ }
          }
          ...
          if (st?.conn?.connected && st.session !== null) {
            try { nav = await navigationState(st.session) } catch { /* 页面切换中 */ }
          }
          json(res, 200, {
            ok: true,
            sessionId,
    ...
    return webServer.register({
      ...
      handler: async (req: any, res: any) => {
        ...
        try {
          ...
          if (st.shellMode) {
            ...
            if (!show) {
              ...
              if (body.keepViewport === true && st.viewport) {
                try { await setViewport(session, st.viewport.width, st.viewport.height) } catch { /* 忽略 */ }
              }
            ...
            try { await clearViewport(session) } catch { /* 未覆写则忽略 */ }
          ...
          const win: any = await conn.send('Browser.getWindowForTarget', { targetId: session.targetId })
          if (win?.windowId == null) return respond(500, { ok: false, error: '无法定位浏览器窗口' })
          const x = Number(body.x)
          const y = Number(body.y)
    ...
    return webServer.register({
      ...
      handler: async (req: any, res: any) => {
        ...
        try {
          ...
          if (st.shellMode) return respond(200, { ok: true, on: body.on === true, shell: true })
          if (body.on === true) {
  ...
  interface BookmarkSite { id: string; title: string; url: string }
  function loadSites(): BookmarkSite[] {
    try {
      const parsed = JSON.parse(fs.readFileSync(sitesFile, 'utf8'))
      if (Array.isArray(parsed?.sites)) {
        return parsed.sites.filter((s: any) => s && typeof s.id === 'string' && typeof s.title === 'string' && typeof s.url === 'string')
      }
    } catch { /* 无文件/损坏 → 默认集 */ }
    return [
  ...
  function saveSites(sites: BookmarkSite[]): void {
    try {
      fs.mkdirSync(dataRoot, { recursive: true })
  ...
  ctx.effect(() => {
    ...
    return webServer.register({
      ...
      handler: async (req: any, res: any) => {
        ...
        try {
          if (req.method === 'GET') return respond(200, { ok: true, sites: loadSites() })
          if (req.method !== 'POST') return respond(405, { ok: false, error: '仅支持 GET/POST' })
          const body = await readJsonBody(req)
          const sites = loadSites()
          if (body?.action === 'add') {
            const title = String(body.title || '').trim().slice(0, 40)
            const url = normalizeSiteUrl(String(body.url || ''))
            if (title === '' || url === null) return respond(400, { ok: false, error: 'title/url 无效' })
            const site: BookmarkSite = { id: Date.now().toString(36) + Math.random().toString(36).slice(2, 6), title, url }
            sites.push(site)
            saveSites(sites)
    ...
    return webServer.register({
      ...
      handler: async (req: any, res: any) => {
        ...
        try {
          ...
          const url = normalizeSiteUrl(String(body.url || ''))
          if (url === null) return respond(400, { ok: false, error: 'url 无效' })
    ...
    return webServer.register({
      ...
      handler: async (req: any, res: any) => {
        ...
        try {
          ...
          const result = await withLock(sessionId, async () => {
            if (action === 'back') return navigateHistory(session, -1)
            if (action === 'forward') return navigateHistory(session, 1)
            if (action === 'reload') return reloadPage(session, body?.hard === true)
      ...
      handler: async (req: any, res: any) => {
        ...
        try {
          if (req.method === 'GET') {
            const sessionId = queryOf(req).get('sessionId') || 'default'
            const st = sessions.get(sessionId)
            if (!st?.shellMode) return respond(200, { ok: true, tabs: [], activeTabId: null, shell: false })
          ...
          const sessionId = String(body?.sessionId || '')
          const st = ensureState(sessionId)
          if (!st.shellMode) return respond(400, { ok: false, error: '当前非壳内视图模式' })
          const action = String(body?.action || '')
          // 标签切换/新建/关闭会改变激活 target；与 AI 工具调用串行，避免工具
          // 在「定位元素」和「点击」之间被换掉页面。
          const result = await withLock(sessionId, async (): Promise<{ status: number; payload: any }> => {
            if (action === 'switch') {
              const tabId = String(body.tabId || '')
              if (!st.tabTargets.has(tabId)) return { status: 404, payload: { ok: false, error: '标签不存在' } }
              st.activeTabId = tabId
              await syncActiveTabSession(st)
              await reattachActiveTab(st)
              tabsCache.delete(st.bvKey)
  ...
  ctx.effect(() => {
    ...
    return webServer.register({
      ...
      handler: async (req: any, res: any) => {
        ...
        try {
          const sid = queryOf(req).get('sessionId')
          if (sid !== null && sid !== '') {
            // 会话级语义（对话面板卡片）：只动内存 deny 集合，绝不写全局配置。
            if (req.method === 'POST') {
              const body = await readJsonBody(req)
              if (!body || typeof body.allow !== 'boolean') return respond(400, { ok: false, error: 'allow 须为布尔值' })
              if (body.allow) sessionDeny.delete(sid)
              else sessionDeny.add(sid)
              return respond(200, { ok: true, sessionId: sid, allow: body.allow })
            }
            return respond(200, { ok: true, sessionId: sid, allow: allowBrowser && !sessionDeny.has(sid) })
          }
          if (req.method === 'POST') {
            // 读 body
            const body = await readJsonBody(req)
