...
function locatorJson(loc: Locator): string {
...
export function locatorLabel(loc: Locator): string {
  if (loc.ref != null) return `ref=${loc.ref}`
  if (loc.selector) return `selector=${loc.selector}`
...
export async function getSnapshot(session: CdpSession): Promise<SnapshotResult> {
  ...
  const refs: Array<{ ref: number; desc: string; off: number }> = data.refs
  if (refs.length === 0) lines.push('  （无可见可交互元素）')
  // 连续同描述折叠为 ref 区间：列表页/表格页省大量重复 token。
  let i = 0
  while (i < refs.length) {
    const cur = refs[i]
    let j = i + 1
    while (j < refs.length && refs[j].desc === cur.desc && refs[j].off === cur.off) j++
    const mark = cur.off === 1 ? ' off-screen' : ''
    if (j - i >= 3) {
      lines.push(`  [${cur.ref}-${refs[j - 1].ref}]（${j - i} 个相同项）${cur.desc}${mark}`)
    } else {
      for (let k = i; k < j; k++) lines.push(`  [${refs[k].ref}] ${refs[k].desc}${mark}`)
    }
    i = j
  }
  if (data.truncated) lines.push(`  （元素过多，已截断至前 ${MAX_REFS} 个；其余用 selector/text 定位或 browser_evaluate）`)
  if (data.bodyText) {
    lines.push('页面正文摘要:')
...
export async function resolveTarget(session: CdpSession, loc: Locator): Promise<ClickTarget> {
  const res = await evaluateJson(session, `${RESOLVE_JS}(${locatorJson(loc)})`, false)
  if (!res) throw new Error('定位元素失败：页面无响应')
  if (res.ok === false) throw new Error(String(res.error || '定位失败'))
  return {
    x: Number(res.x) || 0,
    y: Number(res.y) || 0,
    tag: String(res.tag || ''),
...
export async function clickAt(session: CdpSession, loc: Locator): Promise<ClickTarget> {
  const t = await resolveTarget(session, loc)
  await dispatchMouseClick(session, t.x, t.y)
  return t
}

export async function hoverAt(session: CdpSession, loc: Locator): Promise<ClickTarget> {
  const t = await resolveTarget(session, loc)
...
export async function typeAt(
  ...
  if (!info) throw new Error('聚焦输入元素失败')
...
async function selectValue(session: CdpSession, value: string): Promise<void> {
...
export async function selectAt(session: CdpSession, loc: Locator, value: string): Promise<ClickTarget> {
  const t = await resolveTarget(session, loc)
...
export async function scrollPage(
  ...
  const res = await evaluateJson(
    session,
    `${SCROLL_JS}(${JSON.stringify(direction)}, ${Number(amount) || 3}, ${JSON.stringify(selector ?? '')})`,
    false,
  )
  return {
...
export async function waitForCondition(
  ...
  try {
    ...
    if (!res) return { ok: false, error: '等待失败：页面无响应' }
...
export async function extractContent(
  ...
  const res = await evaluateJson(
    session,
    `${EXTRACT_JS}(${JSON.stringify(selector ?? '')}, ${Math.max(200, Math.min(40000, Number(maxChars) || 6000))})`,
    false,
  )
  if (!res) return { ok: false, error: '提取失败：页面无响应' }
  if (res.ok === false) return { ok: false, error: String(res.error || '提取失败') }
  return {
    ok: true,
    url: String(res.url || ''),
