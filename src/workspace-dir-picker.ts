...
/** 目录选择器业务错误码（镜像官方 directory-picker 语义）。 */
type DirPickerErrorCode = 'directory-unreadable' | 'directory-exists' | 'directory-create-failed'

class DirPickerError extends Error {
  constructor(readonly code: DirPickerErrorCode, readonly path: string, message: string) {
    super(message)
...
function ancestryCrumbs(target: string): Array<{ name: string; path: string; hidden: boolean }> {
...
async function directoryRow(
  ...
  isDirectory: boolean,
  isSymbolicLink: boolean,
): Promise<{ name: string; path: string; hidden: boolean } | null> {
...
async function listLevel(path: string | undefined): Promise<{
  path: string
  home: string
  ...
  const entries: Array<{ name: string; path: string; hidden: boolean }> = []
...
async function createChild(path: string, name: string): Promise<string> {
 ...
 */
export function listDrives(): Array<{ name: string; path: string }> {
...
export function applyWorkspaceDirPicker(ctx: Context): void {
