 ...
 * 两者皆缺时抛错（调度器按失败记录）。
 */
export function resolveRunModel(ctx: Context, job: CronJob): { provider: string, model: string } {
...
function renderOutputDocument(output: string, provider: string, model: string): string {
  return [
 ...
 */
function writeRunOutput(jobId: string, output: string, provider: string, model: string): string | null {
  try {
    const now = new Date()
...
function safeDirId(value: string): string {
  if (!/^[A-Za-z0-9_-]+$/.test(value)) throw new Error(`invalid run dir id: ${value}`)
  return value
}
