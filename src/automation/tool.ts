...
import type { AutomationSuggestionStore, SuggestionJobData } from './suggestions.js'
...
function normalizeSchedule(params: {
  ...
  schedule?: unknown
}, existing: CronJob | null): { type: JobType, schedule: string | number } {
  const rawType = params.scheduleType ?? undefined
  const type = validateType(rawType ?? existing?.type)
  let schedule: unknown = params.schedule ?? existing?.schedule
  ...
  return { type, schedule: schedule as string | number }
}
...
function labelFor(params: { label?: unknown }, prompt: string): string {
  if (typeof params.label === 'string' && params.label.trim() !== '') return params.label.trim()
  return prompt.slice(0, 40)
}

/** 从工具入参构造建议草稿数据。 */
function buildJobData(params: Record<string, unknown>, type: JobType, schedule: string | number): SuggestionJobData {
...
export function registerAutomationTool({ ctx, store, suggestions, scheduler, isAutoApprove }: AutomationToolDeps): () => void {
  const dispose = ctx.tools.register(defineTool({
    ...
    async execute(args, _exec) {
      ...
      const requireId = (): string => {
        ...
        if (id === '') throw new Error(`${String(params.action)} 需要 id 参数（先用 action:"list" 查 id）`)
        return id
      }
      try {
        if (params.action === 'list') {
          const jobs = store.listJobs()
          if (jobs.length === 0) return '当前没有任何定时自动化任务。'
        ...
        if (params.action === 'enable' || params.action === 'disable') {
          ...
          if (current === null) throw new Error(`找不到自动化任务：${id}`)
        ...
        if (params.action === 'run') {
          ...
          if (engine === null) throw new Error('模型服务不可用，当前无法执行任务')
          const current = store.getJob(id)
          if (current === null) throw new Error(`找不到自动化任务：${id}`)
          ...
          if (status === 'error') throw new Error(`任务执行失败：${String(result.error ?? '未知错误')}`)
        ...
        if (params.action === 'runs') {
          ...
          if (runs.length === 0) return `任务「${current.label}」还没有运行记录。`
...
/** 工具调用卡片标题（按动作）。 */
const TOOL_CALL_TITLES: Readonly<Record<string, string>> = {
  list: '查看定时自动化',
  create: '建议新建定时自动化',
  update: '建议修改定时自动化',
  enable: '启用定时自动化',
...
/** 给模型看的运行记录视图。 */
function presentRun(run: RunRecord): Record<string, unknown> {
  return {
    status: run.status,
    at: run.timestamp,
...
/** 给模型看的任务视图（裁掉内部字段噪音）。 */
function presentJob(job: CronJob): Record<string, unknown> {
  return {
    id: job.id,
    label: job.label,
    type: job.type,
...
/** 面向用户的调度描述。 */
function describeSchedule(type: JobType, schedule: string | number): string {
