...
import {
  applyFilePatch, blobSha, isInstalledPath, parseTreeApi, splitPatches,
} from './plugin-update-patch.js'
...
/** 安装形态。 */
type InstallKind = 'pnpm' | 'checkout' | 'unknown'
...
interface InstallInfo {
  kind: InstallKind
  /** profile 目录（pnpm 形态下的更新执行目录）。 */
  profileDir: string | null
  /** profile package.json 里记录的依赖 spec。 */
  spec: string | null
  /** 无法自动更新时给用户的原因说明。 */
  reason: string | null
}
 ...
 * 不开 shell），避免任何注入面。
 */
function run(bin: string, args: readonly string[], cwd: string, timeoutMs: number): Promise<{ ok: boolean; code: number | string; out: string }> {
  return new Promise((resolve) => {
    let out = ''
    let done = false
 ...
 */
function runTool(bin: string, args: readonly string[], cwd: string, timeoutMs: number): Promise<{ ok: boolean; code: number | string; out: string }> {
  if (process.platform !== 'win32') return run(bin, args, cwd, timeoutMs)
...
export function compareVersions(a: string, b: string): number {
  const parse = (v: string): number[] => String(v).replace(/^v/, '').split('-')[0].split('.').map(n => Number(n) || 0)
  const left = parse(a)
  const right = parse(b)
...
/** 带超时的 GET（文本）。 */
async function getText(url: string, accept = '*/*', timeoutMs = FETCH_TIMEOUT_MS): Promise<string> {
...
async function getFirst(
  candidates: ReadonlyArray<{ url: string; accept?: string }>,
  label: string,
 ...
 */
export function parseRefSha(advertisement: string, branch: string): string | null {
 ...
 */
export function parseLockCommit(lock: string, packageName: string): string | null {
  const lines = lock.split(/\r?\n/)
  for (let i = 0; i < lines.length; i += 1) {
    if (!lines[i].includes(packageName)) continue
...
function readLockCommit(profileDir: string, packageName: string): string | null {
  try {
...
function writeMarkerCommit(pkgDir: string, commit: string): void {
  try {
...
/** 读包目录里的增量更新标记（优先于锁文件）。 */
function readMarkerCommit(pkgDir: string): string | null {
  try {
 ...
 */
function readInstalledCommit(pkgDir: string, profileDir: string | null, packageName: string): string | null {
 ...
 */
function detectInstall(local: LocalInfo): InstallInfo {
  ...
  for (const name of names) {
    ...
    if (typeof spec === 'string') return classify(profileDir, spec, local)
  }
  return {
...
function classify(profileDir: string, spec: string | null, local: LocalInfo): InstallInfo {
...
function writeResult(result: UpdateResult): void {
  try {
 ...
 */
function replaceFile(path: string, content: Buffer): void {
...
export function planIncremental(
  ...
  const plan: PatchPlan = { writes: [], deletes: [], skipped: 0 }
  for (const patch of splitPatches(diff)) {
    ...
    if (patch.binary) throw new Error(`${path}: 二进制补丁，增量更新不支持`)
    ...
    if (result === null) throw new Error(`${path}: 补丁上下文不匹配（本地文件可能被改动过）`)
    const content = Buffer.from(result, 'utf8')
    const want = targetTree.get(path)
    if (want === undefined) throw new Error(`${path}: 目标提交里没有该文件的校验值`)
    if (blobSha(content) !== want) throw new Error(`${path}: 打完补丁校验不符（期望 ${want.slice(0, 8)}）`)
 ...
 * @param ctx - host 上下文。
 */
export function applyPluginUpdate(ctx: PluginContext): void {
  let scope: any
  try {
  ...
  const readConfig = (): { autoCheck: boolean; repo: string; branch: string } => {
    let raw: any = {}
    if (scope !== undefined) {
      try { raw = scope.get() ?? {} } catch { raw = {} }
    }
  ...
  if (install.kind === 'checkout') {
    ...
    void run('git', ['rev-parse', 'HEAD'], local.dir, 15000).then((r) => {
      if (r.ok) local.commit = r.out.trim().slice(0, 40)
    })
  } else {
  ...
  const state: {
    ...
    log: string[]
    stage: string | null
    /** 本次更新实际走的路径：增量补丁 / 整包重装（供 UI 明示）。 */
    mode: null | 'incremental' | 'full'
  } = { busy: null, remote: null, error: null, log: [], stage: null, mode: null }
  ...
  const hasUpdate = (): boolean => {
    ...
    if (local.commit !== null && remote.commit !== null) return remote.commit !== local.commit
    if (remote.version === null) return false
    return compareVersions(remote.version, local.version) > 0
  }

  async function check(): Promise<{ ok: boolean; message?: string }> {
    if (state.busy !== null) return { ok: false, message: '已有任务在进行中' }
    state.busy = 'checking'
    state.error = null
    ...
    try {
      ...
      if (commit === null) {
        try {
          ...
          if (typeof json.sha === 'string' && /^[0-9a-f]{40}$/.test(json.sha)) commit = json.sha
        } catch (error: any) {
   ...
   */
  async function updateIncremental(fromCommit: string, toCommit: string): Promise<void> {
    ...
    if (treeJson.truncated === true) throw new Error('目标提交文件树过大被截断，无法校验')
    if (targetTree.size === 0) throw new Error('未能取得目标提交的文件校验表')
    ...
    for (const item of plan.writes) replaceFile(item.path, item.content)
   ...
   */
  async function updateViaPnpm(rawSpec: string, profileDir: string): Promise<void> {
  ...
  async function updateViaGit(): Promise<void> {
    state.stage = 'git'
    ...
    if (head.ok) local.commit = head.out.trim().slice(0, 40)
  }
   ...
   */
  function startUpdate(force: boolean): { ok: boolean; message: string } {
    if (state.busy !== null) return { ok: false, message: '已有任务在进行中' }
    if (install.reason !== null) return { ok: false, message: install.reason }
    if (state.remote === null) return { ok: false, message: '请先点「检查更新」' }
    if (!hasUpdate() && !force) return { ok: false, message: '已是最新版本（如需强制重装，请使用「强制重装最新」）' }
    state.busy = 'updating'
    state.error = null
    state.log = []
    state.mode = null
      ...
      try {
        if (install.kind === 'checkout') {
          ...
          try {
            ...
            state.mode = 'full'
            if (install.spec === null || install.profileDir === null) throw error
  ...
  const handlers: Record<string, (req: any) => Promise<any> | any> = {
    ...
    [`${PLUGIN_UPDATE_API}/check`]: async (req) => {
      ...
      return { ...(snapshot() as object), ...result }
    },
  ...
  ctx.effect(() => {
    const timer = setTimeout(() => {
      if (readConfig().autoCheck) void check()
    }, 15000)
