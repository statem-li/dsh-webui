...
export function listProviders(ctx: AnyContext): ProviderView[] {
  ...
  interface EntryLike { displayName?: unknown, models?: ModelLike[] }
  let raw: unknown
  try {
  ...
  const providersMap = (raw as { providers?: Record<string, EntryLike> } | undefined)?.providers ?? {}
...
export function bindingExists(providers: readonly ProviderView[], binding: ModelBinding): boolean {
  if (binding.provider === '' || binding.model === '') return false
...
export function agentDefaultModel(ctx: AnyContext): ModelBinding | null {
 ...
 */
export function resolveModel({ ctx, team, role, globals, modelOverrides }: ResolveInput): ResolvedModel {
  ...
  if (fallback !== null) return { binding: fallback, source: 'global' }
  throw new TeamError(
    `角色「${role.name}」没有可用模型：请在团队面板设置「团队默认模型」，或为该角色单独指定模型`,
    'model_unresolved', 409,
  )
}

/** 解析并校验模型存在性；不存在时给出可操作的错误提示。 */
export function resolveModelChecked(input: ResolveInput, providers: readonly ProviderView[]): ResolvedModel {
  ...
  if (providers.length > 0 && !bindingExists(providers, resolved.binding)) {
    ...
    throw new TeamError(
      `${where} ${resolved.binding.provider}/${resolved.binding.model} 不在已配置的供应商中，请到团队设置里重选`,
      'model_missing', 409,
    )
  }
  return resolved
...
export function assertTeamRunnable(team: Team, chain: Chain | null): void {
