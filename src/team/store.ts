...
/** 数据根：${DSH_HOME:-~/.dsh}/team/。 */
export function teamDataRoot(): string {
...
export class TeamStore {
  ...
  private teamPath(id: string): string {
  ...
  readTeam(id: string): Team {
    if (!isValidTeamId(id)) throw new TeamError(`团队 id 非法：${id}`, 'team_id_invalid', 400)
    const path = this.teamPath(id)
    if (!existsSync(path)) throw new TeamError(`团队不存在：${id}`, 'team_not_found', 404)
    let raw: unknown
    try {
      raw = readJson(path)
    } catch {
    ...
    if (team.schemaVersion > TEAM_SCHEMA_VERSION) {
      throw new TeamError(
        `团队 ${id} 的存储版本（v${team.schemaVersion}）高于当前插件支持的 v${TEAM_SCHEMA_VERSION}，已按只读处理`,
        'team_version_ahead', 409,
      )
    }
    return team
  }
  ...
  /** 读团队，失败返回 null（列表投影用）。 */
  tryReadTeam(id: string): { team: Team } | { issue: string } {
    try {
  ...
  saveTeam(input: unknown): Team {
    ...
    const next: Team = { ...team, schemaVersion: TEAM_SCHEMA_VERSION, updatedAt: new Date().toISOString() }
    writeJsonAtomic(this.teamPath(next.id), next)
    return next
  }

  /** 新建团队：seed=true 用出厂编制，否则空白团队（只含主脑 + 一条空链）。 */
  createTeam(name: string, options: { seed?: boolean, id?: string } = {}): Team {
  ...
  duplicateTeam(sourceId: string, name?: string): Team {
    ...
    const copy: Team = {
      ...JSON.parse(JSON.stringify(source)) as Team,
      id,
      name: nextName,
      createdAt: now,
      updatedAt: now,
  ...
  removeTeam(id: string): { removed: string, activeTeamId: string } {
    const path = this.teamPath(id)
    if (!existsSync(path)) throw new TeamError(`团队不存在：${id}`, 'team_not_found', 404)
    ...
    for (const [sid, mode] of Object.entries(all)) {
      if (mode.teamId === id) { all[sid] = { ...mode, teamId: '' }; changed = true }
    }
    if (changed) writeJsonAtomic(this.chatModePath, all)
    const globals = this.readGlobals()
    if (globals.activeTeamId !== id) return { removed: id, activeTeamId: globals.activeTeamId }
  ...
  /** 由名称派生一个未被占用的团队 id。 */
  private allocTeamId(name: string): string {
    const ascii = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '')
    const base = ascii !== '' ? `t-${ascii}`.slice(0, 40) : `t-${Date.now().toString(36)}`
    if (!existsSync(this.teamPath(base))) return base
    for (let i = 2; i < 500; i += 1) {
      const candidate = `${base}-${i}`
      if (!existsSync(this.teamPath(candidate))) return candidate
    }
  ...
  resolveTeam(id?: string): Team {
    if (id !== undefined && id !== '') return this.readTeam(id)
    const globals = this.readGlobals()
    if (globals.activeTeamId !== '') {
      const result = this.tryReadTeam(globals.activeTeamId)
      if ('team' in result) return result.team
    }
  ...
  readChatModes(): Record<string, ChatModeState> {
    try {
      ...
      const out: Record<string, ChatModeState> = {}
  ...
  readChatMode(sessionId: string): ChatModeState {
    if (sessionId === '') return { ...DEFAULT_CHAT_MODE }
  ...
  writeChatMode(sessionId: string, patch: Partial<ChatModeState>): ChatModeState {
    if (sessionId === '') throw new TeamError('缺少 sessionId', 'session_required', 400)
    const all = this.readChatModes()
  ...
  sessionActiveTeamId(sessionId: string): string {
    if (sessionId !== '') {
      ...
      if (teamId !== '' && existsSync(this.teamPath(teamId))) return teamId
    }
  ...
  /** 运行清单（可按团队/会话过滤）。 */
  listRuns(options: { teamId?: string, sessionId?: string, limit?: number } = {}): RunSummary[] {
    ...
    for (const id of this.listRunIds()) {
      ...
      if (options.teamId !== undefined && options.teamId !== '' && run.teamId !== options.teamId) continue
      if (options.sessionId !== undefined && options.sessionId !== '' && run.sessionId !== options.sessionId) continue
  ...
  trimRuns(): void {
    ...
    for (const id of ids.slice(RUNS_KEEP)) {
      try { this.removeRun(id) } catch { /* ignore */ }
    }
  }
