...
import { TeamError, TEAM_SCHEMA_VERSION, type Chain, type DirectLink, type Role, type Team } from './types.js'
...
function extractJson(text: string): Record<string, unknown> {
  ...
  try {
    ...
    throw new TeamError(
      `生成结果 JSON 解析失败：${error instanceof Error ? error.message : String(error)}`,
      'gen_bad_json', 502,
    )
  }
...
function resolveGenModel(
  ctx: AnyContext,
  store: TeamStore,
  explicit: { provider?: string, model?: string } | undefined,
  ...
  throw new TeamError(
    '没有可用于生成的模型：请在团队设置里选一个「全局默认模型」，或在生成时指定模型',
    'gen_no_model', 409,
  )
}
...
function sanitizeChains(input: unknown, roles: readonly Role[]): Chain[] {
  ...
  for (const item of list) {
    ...
    const raw = item as Record<string, unknown>
    let id = typeof raw.id === 'string' ? raw.id.trim().toLowerCase().replace(/[^a-z0-9_-]/g, '') : ''
    if (id === '') id = `chain${chains.length + 1}`
    id = id.slice(0, 30)
    if (seen.has(id)) continue
    seen.add(id)
    const steps = (Array.isArray(raw.steps) ? raw.steps : [])
      .map((step) => {
        ...
        if (s.kind === 'synthesize') return { kind: 'synthesize' as const }
  ...
  if (chains.length === 0) {
    ...
    if (workers.length > 0) {
      chains.push({
        ...
        steps: workers.map(role => ({ kind: 'role' as const, roleId: role.id })),
...
/** 校验并裁剪直连。 */
function sanitizeLinks(input: unknown, roles: readonly Role[]): DirectLink[] {
  const ids = new Set(roles.map(role => role.id))
  const list = Array.isArray(input) ? input : []
...
export async function generateTeam(
  ...
  if (brief === '') throw new TeamError('请先描述你想要的团队', 'brief_required', 400)
  if (brief.length > 2000) throw new TeamError('需求描述过长（上限 2000 字）', 'brief_too_long', 400)

  const llm = ctx.get?.('llm') as LlmLike | undefined
  if (llm === undefined) throw new TeamError('llm 服务不可用，无法生成团队', 'llm_unavailable', 503)
