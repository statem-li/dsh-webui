...
const RULES: ReadonlyArray<{ kind: StepErrorKind, patterns: readonly RegExp[] }> = [
  {
    kind: 'cancelled',
...
export function classifyFailure(message: string): StepErrorKind {
...
export function failureLabel(kind: StepErrorKind): string {
  switch (kind) {
...
export function failureAdvice(kind: StepErrorKind): string {
  switch (kind) {
...
/** 原地重试是否有意义（同一个模型再试一次）。 */
export function isRetryable(kind: StepErrorKind): boolean {
  switch (kind) {
    case 'rate_limit':
    case 'timeout':
    case 'network':
...
export function shouldFallback(kind: StepErrorKind): boolean {
  switch (kind) {
 ...
 * 命中返回毫秒，未命中返回 null。
 */
export function retryAfterHint(message: string): number | null {
  const text = message.toLowerCase()
  const patterns: ReadonlyArray<{ re: RegExp, unit: number }> = [
 ...
 */
export function backoffMs(kind: StepErrorKind, attempt: number, hintMs?: number | null): number {
