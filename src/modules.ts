...
const KEY_SET: ReadonlySet<string> = new Set(WEBUI_MODULE_KEYS)

/**
 * 从任意来源（settings 值 / localStorage JSON / API body）提取合法的部分
 * 覆盖表：丢弃未知 key 与非布尔值，绝不抛错。
 */
export function normalizeModules(input: unknown): WebuiModuleOverrides {
  const out: WebuiModuleOverrides = {}
 ...
 */
export function isModuleEnabled(modules: WebuiModuleOverrides, key: WebuiModuleKey): boolean {
