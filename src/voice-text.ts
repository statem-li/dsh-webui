 ...
 */
export const SUMMARY_TARGET_CHARS = 90
 ...
 * @returns 句子列表与残句。
 */
export function segmentSentences(input: string, options: { final?: boolean } = {}): Segmented {
  ...
  for (let index = 0; index < text.length; index += 1) {
    ...
    while (index + 1 < text.length && /["'）」』】\)\]]/.test(text[index + 1] as string)) {
      index += 1
      buffer += text[index]
    }
...
/** 一句话里线索词命中数。 */
function hitCount(sentence: string, hints: readonly string[]): number {
  const lower = sentence.toLowerCase()
  let count = 0
  for (const hint of hints) if (lower.includes(hint)) count += 1
  return count
}
 ...
 */
export function outcomeSummary(text: string, limit = SUMMARY_TARGET_CHARS): string {
  ...
  if (sentences.length === 0) return clampSpeech(body, limit)
  ...
  for (const item of picked) {
    if (out !== '' && out.length + item.sentence.length > limit) break
    out += item.sentence
 ...
 * @returns 截断后的文本。
 */
export function clampSpeech(text: string, limit = SPEECH_MAX_CHARS): string {
