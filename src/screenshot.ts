...
import { renderMarkdownToHtml, buildThemeCss, type ShotTheme } from './markdown-html.js'
...
function messageHeading(text: string, role: 'user' | 'assistant'): string {
...
async function renderMessageHtml(role: 'user' | 'assistant', text: string, theme: ShotTheme): Promise<string> {
...
export async function captureHtml(html: string): Promise<string> {
...
export function withShotLock<T>(task: () => Promise<T>): Promise<T> {
...
async function handle(ctx: Context, req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  const theme: ShotTheme = body.theme === 'dark' || body.theme === 'glass' || body.theme === 'glass-dark' ? body.theme as ShotTheme : 'light'
  if (role === '' || text.trim() === '') {
