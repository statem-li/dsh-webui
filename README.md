...
31. **自动化**：左侧导航「新会话」下方菜单项，点开从菜单右侧滑出 TAB 式卡片（窄屏回退底部 sheet），卡片宽高随 TAB 平滑过渡——「定时」：执行日期 + 每天定时开关；「执行任务」：按分类管理任务，二级右侧抽屉新建/编辑任务并可选模型与推理强度（数据来自 DSH 模型目录）；「执行日志」：大卡片按日期倒序展示每天执行记录，支持按任务下拉筛选与清空；定时条件满足时自动落当日记录（同任务同日去重），240ms 淡入淡出 + 内容错落渐显，数据 localStorage 持久化（v1 配置自动迁移）
