...
export function applyVisionHelper(ctx, configInput) {
        ...
        try {
            ...
            for (const [key, value] of Object.entries(caps)) {
                if (Array.isArray(value)) {
                    const clean = value.filter((x) => typeof x === 'string' && (x === 'image' || x === 'video'));
                    if (clean.length > 0)
                        out[key] = clean;
                }
            }
                    ...
                    if (req.method === 'POST') {
                        ...
                        for (const [key, value] of Object.entries(caps)) {
                            if (splitKey(key) && Array.isArray(value)) {
                                const mods = value.filter((x) => typeof x === 'string' && (x === 'image' || x === 'video'));
                                if (mods.length > 0)
                                    clean[key] = mods;
                            }
                        }
    ...
    async function testCapability(provider, model, capability, signal) {
        if (capability === 'vision') {
            ...
            try {
                const res = await callVisionChat(ctx, profile.baseURL, apiKey, model, TEST_IMAGE_PNG, '这张图是什么颜色？用一句话回答。', config.timeoutMs, signal, { flatImage: false, reasoningOff: null, maxTokens: config.maxTokens });
                if (res.ok && typeof res.content === 'string' && res.content.trim().length > 0) {
