...
import { TeamError, normalizeBinding } from './types.js';
...
function syncSettingsToGlobals(scope, store) {
    if (scope === undefined)
        return;
    let value;
    try {
    ...
    if (typeof value.activeTeamId === 'string' && value.activeTeamId.trim() !== '')
        patch.activeTeamId = value.activeTeamId.trim();
    for (const key of ['timeoutSec', 'maxRetries', 'maxConcurrentRuns', 'maxParallel', 'outputChunkChars']) {
        if (typeof value[key] === 'number')
            patch[key] = value[key];
    }
    if (typeof value.stopOnError === 'boolean')
        patch.stopOnError = value.stopOnError;
    if (typeof value.autoFallback === 'boolean')
        patch.autoFallback = value.autoFallback;
    if (typeof value.autoPlan === 'boolean')
        patch.autoPlan = value.autoPlan;
    if (Object.keys(patch).length === 0)
        return;
    try {
        store.patchGlobals(patch);
    }
    catch { /* ignore */ }
}
async function handleTeams(deps, req, res, url) {
    const { store } = deps;
    ...
    if (req.method === 'POST') {
         ...
         * 不带（旧调用方/工具）回退写全局默认。
         */
        const setCurrent = (teamId) => {
            if (session !== '')
                store.setSessionActiveTeam(session, teamId);
            else
                store.patchGlobals({ activeTeamId: teamId });
        };
        if (action === 'create') {
...
async function handleTeamDetail(deps, teamId, req, res) {
    const { store } = deps;
    if (req.method === 'POST') {
        const body = await readBody(req);
        const payload = (body.team !== undefined ? body.team : body);
...
async function handleGlobals(deps, req, res) {
    const { store } = deps;
    if (req.method === 'POST') {
        const body = await readBody(req);
        const patch = { ...(body.globals !== undefined ? body.globals : body) };
...
async function handleChatMode(deps, url, req, res) {
    const { store } = deps;
    if (req.method === 'POST') {
        const body = await readBody(req);
        ...
        if (typeof body.teamId === 'string')
            patch.teamId = body.teamId.trim();
        if (typeof body.chainId === 'string')
            patch.chainId = body.chainId.trim();
        if (typeof body.force === 'boolean')
            patch.force = body.force;
...
async function handleRuns(deps, url, req, res) {
    ...
    if (req.method === 'POST') {
        ...
        const roles = Array.isArray(body.roles)
            ? body.roles.filter((r) => typeof r === 'string' && r.trim() !== '').map(r => r.trim())
            : [];
        const overrides = {};
        if (body.modelOverrides !== null && typeof body.modelOverrides === 'object') {
            for (const [roleId, value] of Object.entries(body.modelOverrides)) {
                const binding = normalizeBinding(value);
                if (binding !== null && binding.provider !== '' && binding.model !== '')
                    overrides[roleId] = binding;
            }
        }
        const sessionId = typeof body.sessionId === 'string' ? body.sessionId.trim() : '';
...
function handleActiveRuns(deps, url, res) {
    const { store } = deps;
    const sessionId = url.searchParams.get('sessionId') ?? '';
    // 会话严格隔离：只返回本会话的活跃运行。
    ...
    const recent = sessionId !== ''
        ? store.listRuns({ limit: 12 }).filter(summary => summary.status !== 'running' && summary.status !== 'queued' && summary.sessionId === sessionId)
        : [];
 ...
 */
function withRunFlags(run) {
    return { ...run, progress: runProgress(run), resumable: isResumable(run) };
}
/** 校验调用方对本运行的会话归属；无归属的旧运行放行（跨会话禁止操作）。 */
function assertRunOwnership(store, runId, sessionId) {
    const run = store.readRun(runId);
    if (run === null)
        throw new TeamError(`找不到运行：${runId}`, 'run_not_found', 404);
...
async function handleRunDetail(deps, runId, tail, url, req, res) {
    const { store, engine } = deps;
    const sessionId = url.searchParams.get('sessionId') ?? '';
...
// ── 装配 ────────────────────────────────────────────────────────────────────
/** 挂载 team 模块（host 半身）。 */
export function applyTeamHost(ctx) {
    ...
    ctx.effect(() => webServer.register({
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                if (parts[0] === 'teams') {
                    if (parts.length === 1)
                        return await handleTeams(deps, req, res, url);
                    return await handleTeamDetail(deps, parts[1], req, res);
                }
                if (parts[0] === 'globals')
                    return await handleGlobals(deps, req, res);
                ...
                if (parts[0] === 'chat-mode')
                    return await handleChatMode(deps, url, req, res);
                if (parts[0] === 'runs') {
                    if (parts.length === 1)
                        return await handleRuns(deps, url, req, res);
                    if (parts[1] === 'active')
                        return handleActiveRuns(deps, url, res);
