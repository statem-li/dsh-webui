...
peers = []) {
    ...
    if (peers.length > 0) {
        head.push('', '## 并行说明', `本步与「${peers.join('、')}」**同时进行**：你看不到他们的产出，他们也看不到你的。`, '- 只做你职责范围内的部分，不要替他们的职责下结论、不要复述他们大概会说什么。', '- 需要他们的结论才能推进的部分，写成「待汇合确认：…」交给后续整合处理。');
    }
...
/** 编排 prompt 的用户消息。 */
export function buildPlanPrompt(team, task, maxParallel) {
    const roster = team.roles
