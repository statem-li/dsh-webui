...
import { TeamError, effectiveGlobals, normalizePlan, } from './types.js';
import { assertTeamRunnable, describePlan, findCoreRole, isResumable, listProviders, planChain, planResume, planRoles, planWaves, resolveCandidates, resolveModelChecked, waveCountOf, } from './roster.js';
import { backoffMs, classifyFailure, failureLabel, isRetryable, retryAfterHint, shouldFallback, } from './failure.js';
import { PLAN_SYSTEM, buildPlanPrompt, buildSystem, buildUserPrompt, renderFinalDocument, renderStepDocument, } from './prompts.js';
...
export class TeamEngine {
    ...
    async execute(runId, team, initialPlan, globals, context, options) {
        ...
        if (options.autoPlan) {
            try {
                ...
                if (planned.length === 0) {
                    ...
                    planned = planRoles(team, team.roles.filter(role => role.group !== 'core').map(role => role.id), options.synthesize);
                }
                run = {
    ...
    resume(runId, context = {}) {
        ...
        if (planned.length === 0) {
            throw new TeamError(missing.length > 0
                ? `未完成步骤的角色已从团队中删除（${missing.join('、')}），无法接续`
                : '没有可接续的步骤', 'resume_empty', 409);
        }
     ...
     * 全部无效时返回空波次由调用方兜底。
     */
    async askForPlan(team, task, globals, signal, providers) {
        const core = findCoreRole(team);
        if (core === null)
            throw new TeamError('团队没有主脑角色（core 分组），无法自主编排', 'no_core_role', 409);
        ...
        let text = '';
        try {
            text = await this.invokeLlm({ ...binding, maxTokens: PLAN_MAX_TOKENS }, PLAN_SYSTEM, buildPlanPrompt(team, task, globals.maxParallel), stepController.signal, () => { });
        }
        finally {
            clearTimeout(timer);
    ...
    /** 调用一次通道（统一超时 + 取消语义）。 */
    async invoke(args) {
        const { channel, binding, system, userPrompt, globals, controller, context, toolFilter, label, runId, stepIndex, onDelta, onPhase, } = args;
        const stepController = new AbortController();
        const onAbort = () => stepController.abort();
    ...
    async invokeLlm(binding, system, userPrompt, signal, onDelta, onPhase) {
...
function groupByWave(planned) {
    const buckets = new Map();
    for (const step of planned) {
        const list = buckets.get(step.wave);
        if (list === undefined)
            buckets.set(step.wave, [step]);
        else
            list.push(step);
    }
