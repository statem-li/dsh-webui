...
function runCmd(bin, args, cwd, timeoutMs = 120000) {
    return new Promise((resolve) => {
        execFile(bin, args, {
            ...
            if (err) {
                resolve({ ok: false, code: err.code ?? 'error', stdout: String(stdout ?? ''), stderr: String(stderr ?? err.message) });
            }
            else {
...
function logResult(label, r) {
    ...
    if (err)
        parts.push('stderr: ' + outTail(err, 2000));
    ...
    async function checkUpdate() {
        ...
        try {
            ...
            const dirty = dirtyRes.ok
                ? dirtyRes.stdout.split(/\r?\n/).filter(Boolean).map(porcelainPath).filter((p) => !!p && isSourcePath(p)).length
                : 0;
    ...
    async function runUpdate(stashed) {
        ...
        try {
            ...
            if (stashed) {
                ...
                if (!popRes.ok)
                    appendLog('stash pop 有冲突，请手动处理（git stash list）');
            }
            // 5) 记录结果
