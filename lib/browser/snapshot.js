...
export async function getSnapshot(session) {
    ...
    for (const r of data.refs)
        lines.push(`  [${r.ref}] ${r.desc}`);
    if (data.truncated)
        lines.push(`  （元素过多，已截断至前 ${MAX_REFS} 个，其余用 browser_evaluate 定位）`);
    if (data.bodyText) {
        lines.push('页面正文摘要:');
...
export async function clickRef(session, ref) {
    const rect = await evaluateJson(session, `${GET_RECT_JS}(${Number(ref)})`);
    if (!rect)
        throw new Error('定位元素失败');
    if (rect.ok === false)
        throw new Error(String(rect.error || '点击失败'));
...
export async function hoverRef(session, ref) {
    const rect = await evaluateJson(session, `${GET_RECT_JS}(${Number(ref)})`);
    if (!rect)
        throw new Error('定位元素失败');
    if (rect.ok === false)
        throw new Error(String(rect.error || '悬停失败'));
...
export async function typeRef(session, ref, text, pressEnter) {
    const info = await evaluateJson(session, `${FOCUS_SELECT_JS}(${Number(ref)})`);
    if (!info)
        throw new Error('定位输入元素失败');
    if (info.ok === false)
...
export async function selectRef(session, ref, value) {
...
export async function scrollPage(session, direction, amount) {
