...
export async function captureScreenshot(session, quality = 90, format = 'jpeg') {
    const { conn, sessionId } = session;
    const shot = await conn.send('Page.captureScreenshot', { format, ...(format === 'jpeg' ? { quality } : {}), fromSurface: true }, sessionId);
    if (!shot?.data)
