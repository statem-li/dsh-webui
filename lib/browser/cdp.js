...
export async function captureScreenshot(session, quality = 90, format = 'jpeg', fromSurface = true, timeoutMs = 8000) {
    const { conn, sessionId } = session;
    const shot = await conn.send('Page.captureScreenshot', { format, ...(format === 'jpeg' ? { quality } : {}), fromSurface }, sessionId, timeoutMs);
    if (!shot?.data)
...
export async function inspectElementAt(session, x, y) {
    ...
    const empty = { found: false, selector: '', tag: '', id: '', className: '', role: '', text: '', label: '' };
