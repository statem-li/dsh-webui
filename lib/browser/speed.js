...
const SPEED_INSTRUCTION = [
    ...
    '2) 需要真实事件语义的连续操作用 browser_batch 合并执行（单次最多 20 个动作，支持 click/type/select/hover/press/scroll/navigate/wait），只在任务关键节点查看快照；',
    '3) 能拼出最终 URL 就直接 browser_navigate 打开目标页，不做「首页→搜索→点结果」式逐跳导航；navigate 可带 wait_for_selector / wait_for_text，加载与等待合成一次调用；',
    '4) 元素定位优先用 selector（CSS）或 text_match（可见文本）——它们无需先 browser_snapshot，页面变化后也不必重拍快照，能直接省掉整轮推理；只有需要看清页面结构时才 snapshot；',
    '5) 等页面出结果用 browser_wait_for（等选择器/文本出现，或 gone 等加载遮罩消失），等待在页面内轮询完成，不要用「反复 snapshot 看有没有变」空转；',
    '6) 只想读内容（文章正文、搜索结果、表格）用 browser_extract，比 snapshot 省 token 且无交互元素噪音；',
    '7) 尽量不截图；确需看画面细节时（视觉核对/图表/验证码/样式确认），browser_screenshot 传 selector 只截目标元素区域再 vision_describe 该路径，不要整页截图；',
    '8) 仅当下一步确实取决于上一步的页面反馈、或单次独立操作时才逐步调用。',
].join('\n');
...
export function applyBrowserSpeed(ctx) {
