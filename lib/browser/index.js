...
import { CdpConnection, attachTarget, listPageTargets, navigateAndWait, navigateHistory, waitForPageReady, captureScreenshot, captureScreenshotSafe, captureScreenshotSafeClip, fetchBrowserWsUrl, evaluateJson, inspectElementAt, getViewportSize, setViewport, clearViewport, dispatchKey, dispatchMouseMove, dispatchMouseClick, dispatchMouseButton, dispatchMouseWheel, insertText, startScreencast, stopScreencast, } from './cdp.js';
            ...
            async execute(args, exec) {
                ...
                try {
                    ...
                    if (selector !== '') {
                        ...
                        let rect;
                        try {
                            rect = typeof raw === 'string' ? JSON.parse(raw) : raw;
                        }
                        catch {
                            return { ok: false, error: '元素矩形解析失败' };
                        }
