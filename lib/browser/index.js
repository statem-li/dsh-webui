...
import { CdpConnection, createPageSession, navigateAndWait, navigateHistory, waitForPageReady, captureScreenshot, setViewport, fetchBrowserWsUrl, evaluateJson, dispatchKey, dispatchMouseMove, dispatchMouseClick, insertText, startScreencast, stopScreencast, ackScreencast, dispatchMouseWheel, dispatchMouseButton, } from './cdp.js';
import { resolveChromePath, launchChrome, killChrome, findFreePort, profileDirFor, DEFAULT_CHROME_CANDIDATES, } from './chrome.js';
    ...
    async function startBrowserFor(sessionId) {
        ...
        try {
            ...
            st.offFrame = conn.on('Page.screencastFrame', (p) => {
                if (!p || typeof p.data !== 'string' || p.sessionId !== session.sessionId)
                    return;
                ...
                try {
                    if (req.method === 'POST') {
                        ...
                        if (!body || typeof body.headless !== 'boolean')
                            return respond(400, { ok: false, error: 'headless 须为布尔值' });
