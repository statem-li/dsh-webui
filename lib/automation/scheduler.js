        ...
        const run = (async () => {
            ...
            try {
                ...
                if (isSkippedError(error)) {
                    const record = { status: 'skipped', startedAt, finishedAt, reason: message, trigger };
                    store.logRun(job.id, record);
                    const result = { ...record };
                    onJobDone?.(job, result);
                    return result;
    ...
    async function doCheck() {
        try {
            ...
            for (const listedJob of jobs) {
                ...
                if (!Number.isFinite(nextRunTime) || Date.now() < nextRunTime)
                    continue;
