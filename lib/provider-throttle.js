    ...
    acquire(timeoutMs, signal) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const onAbort = () => {
                ...
                if (idx >= 0)
                    this.queue[idx] = null; // 占位保留，release 时顺手清掉
                cleanup();
                reject(abortError());
            };
...
function rateLimitedResponse() {
    ...
    return new Response(JSON.stringify({ error: { message: 'rate limit exceeded: provider throttle (429)', type: 'rate_limit_error', code: 'provider_throttled' } }), {
        status: 429,
 ...
 */
async function runThrottled(entry, task, signal) {
...
function installHook() {
    ...
    globalThis.fetch = function (input, init) {
        ...
        const signal = init !== null && init !== undefined && typeof init === 'object' ? init.signal : undefined;
...
// ── 应用入口 ──────────────────────────────────────────────────────────────
export function applyProviderThrottle(ctx) {
    let scope;
    try {
...
//# sourceMappingURL=provider-throttle.js.map