...
export function isDuplicateContent(content, existing, threshold = DUP_SIMILARITY_THRESHOLD) {
    ...
    for (const entry of existing) {
        ...
        if (entry.deprecated === true)
            continue;
        if (entry.content.trim() === trimmed)
            return true;
        if (semanticSimilarity(trimmed, entry.content) >= threshold)
            return true;
    }
    return false;
}
