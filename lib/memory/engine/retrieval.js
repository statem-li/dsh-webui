 ...
 */
import { normalizedCosine } from './embedding.js';
/** 归一化：小写 + 折叠空白。 */
...
export function searchEntries(query, entries, mode, options = {}) {
 ...
 */
export async function searchEntriesSemantic(query, entries, provider, options = {}) {
    const active = options.includeDeprecated === true
        ? entries
        : entries.filter(entry => entry.deprecated !== true);
    if (active.length === 0)
