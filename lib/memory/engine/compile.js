...
export function isIdentityEntry(entry) {
    if (entry.scope !== 'global')
        return false;
    if (entry.kind === 'identity' || entry.kind === 'preference')
        return true;
...
export function isFactEntry(entry) {
    ...
    if (entry.kind === 'identity' || entry.kind === 'preference')
        return false;
    if (entry.kind === 'fact' || entry.kind === 'decision' || entry.kind === 'gotcha')
        return true;
...
export function buildInjectionText(entries, config) {
    ...
    for (const entry of pinned)
        buckets.pinned.push(entryLine(entry));
    ...
    for (const entry of rest) {
        const section = entry.scope === 'global'
        ...
        if (used + line.length + 1 + headerCost > budget)
            continue;
