...
import { isSensitiveContent, resolveRoute } from './extract.js';
import { entryIdOf, nowIso, summarize } from './store.js';
/** 整理用伪 agent：无显式 provider/model，resolveRoute 回退默认模型。 */
const CONSOLIDATE_AGENT = {
 ...
 */
export async function consolidateScope(ctx, store, config, scope, trigger) {
    ...
    try {
        ...
        for (const event of events)
            await store.appendChange(event);
...
export async function consolidateAll(ctx, store, config, trigger) {
    if (!config.consolidateEnabled)
        return [];
    ...
    for (const op of ops) {
        switch (op.type) {
            case 'merge': {
                ...
                // 仍然删除源条目——目标内容已经在库里。
                if (!byId.has(merged.id) || removeIds.has(merged.id))
            ...
            case 'rewrite': {
                ...
                if (entry === undefined || entry.pinned)
                    break;
                const content = sanitizeContent(op.content);
                if (content === '' || isSensitiveContent(content) || content === entry.content)
                    break;
                ...
                if (nextId !== entry.id && byId.has(nextId))
                    break;
            ...
            case 'drop': {
                for (const id of op.ids) {
                    ...
                    if (entry === undefined || entry.pinned)
                        continue;
                    if (removeIds.has(id) || removeObjects.has(entry))
                        continue;
                    removeIds.add(id);
            ...
            case 'promote': {
                for (const id of op.ids) {
                    const entry = byId.get(id);
                    if (entry === undefined || entry.layer === 'long')
                        continue;
                    entry.layer = 'long';
                    entry.updatedAt = nowIso();
                    events.push({
...
export function parseConsolidateOutput(raw) {
    let text = raw.trim();
    ...
    for (const item of ops) {
        ...
        if (type !== 'merge' && type !== 'rewrite' && type !== 'drop' && type !== 'promote')
            continue;
        const ids = Array.isArray(record.ids)
            ? record.ids.filter((id) => typeof id === 'string' && id.trim() !== '').map(id => id.trim())
...
function sanitizeTags(value) {
    return Array.isArray(value)
        ? value.filter((tag) => typeof tag === 'string' && tag.trim() !== '').map(tag => tag.trim()).slice(0, 8)
        : [];
}
