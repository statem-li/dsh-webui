...
export function registerMemoryTools(ctx, store, _config) {
    const disposers = [];
    // ── memory_search ────────────────────────────────────────────────────
    disposers.push(ctx.tools.register(textTool({
        ...
        async execute(args, exec) {
            ...
            const views = entries
                .map(toView)
                .filter(view => {
                if (view.scope === 'project' && projectFilter !== null && view.projectHash !== projectFilter)
                    return false;
                if (typeof args.scope === 'string' && view.scope !== args.scope)
                    return false;
                if (typeof args.tag === 'string' && args.tag !== '' && !view.tags.includes(args.tag))
                    return false;
                if (typeof args.query === 'string' && !matchesQuery(view, args.query))
                    return false;
                return true;
            })
                .sort(rank);
