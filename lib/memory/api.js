...
import { applyConfigOverrides, DEFAULT_CONFIG, publicConfig } from './types.js';
...
async function handle(ctx, store, config, req, res) {
    ...
    if (config.logApiRequests)
        void store.appendApiLog(`${method} ${rest} start`).catch(() => undefined);
    try {
        // ── 查询 ──────────────────────────────────────────────────────────
        ...
        if (method === 'POST' && rest === '/update') {
            ...
            if (isMemoryKindValue(body.kind))
                patch.kind = body.kind;
            if (body.layer === 'short' || body.layer === 'long')
                patch.layer = body.layer;
        ...
        if (method === 'POST' && rest === '/move') {
            ...
            if (body.scope === 'global') {
                ...
                if (projectHash === null)
                    throw new Error('移入项目需要 projectHash 或 path');
        ...
        if (method === 'POST' && rest === '/delete-batch') {
            ...
            const ids = Array.isArray(body.entryIds)
                ? body.entryIds.filter((id) => typeof id === 'string' && id.trim() !== '').map(id => id.trim())
                : [];
            if (ids.length === 0)
                throw new Error('entryIds 不能为空');
        ...
        if (method === 'POST' && rest === '/rollback') {
            ...
            if (!ok)
                throw new Error(`修订不存在：${revisionId}`);
            await compileAll(store, config);
        ...
        if (config.logApiRequests)
            void store.appendApiLog(`${method} ${rest} done ${Date.now() - apiStarted}ms`).catch(() => undefined);
    }
}
...
async function listView(store, params) {
    ...
    const scoped = entries.filter(entry => {
        ...
        if (tag !== null && tag !== '' && !entry.tags.includes(tag))
            return false;
        if (!includeDeprecated && entry.deprecated === true)
            return false;
        return true;
    });
