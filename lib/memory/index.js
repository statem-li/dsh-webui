...
function resolveConfig(input) {
    ...
    const numbers = ['extractEveryTurns', 'compileEveryTurns', 'compileThreshold', 'decayLambda', 'hitBonus', 'injectTokenBudget', 'injectRefreshSteps', 'extractMaxChars', 'minImportance', 'consolidateMaxEntries', 'consolidateTimeoutMs'];
...
/** 应用入口。 */
export function applyMemory(ctx, input) {
    ...
    function uncaughtListener(error) { logError('uncaughtException', error); }
    function unhandledListener(reason) { logError('unhandledRejection', reason); }
...
async function extractTurn(ctx, store, config, agent, buffer, turnNumber) {
    ...
    if (config.extractEveryTurns > 1 && turnNumber % config.extractEveryTurns !== 1)
        return;
    ...
    if ((sessionState?.extractFailStreak ?? 0) >= 3 && turnNumber % 10 !== 1)
        return;
