...
export function decodePng(buf) {
    ...
    if (bitDepth !== 8)
        throw new Error(`不支持的 PNG 位深: ${bitDepth}`);
    if (colorType !== 2 && colorType !== 6)
        throw new Error(`不支持的 PNG 颜色类型: ${colorType}`);
    if (interlace !== 0)
        throw new Error('不支持的 PNG 交织模式');
    if (width <= 0 || height <= 0)
        throw new Error('PNG 缺少合法的 IHDR');
    const bpp = colorType === 6 ? 4 : 3;
    const stride = width * bpp;
    const raw = inflateSync(Buffer.concat(idat));
    const expected = (stride + 1) * height;
    if (raw.length < expected)
        throw new Error(`PNG 数据不完整: ${raw.length} < ${expected}`);
 ...
 */
export function encodePng(width, height, pixels, bpp) {
    if (width <= 0 || height <= 0)
        throw new Error('编码尺寸非法');
    if (bpp !== 3 && bpp !== 4)
        throw new Error(`不支持的 bpp: ${bpp}`);
    const stride = width * bpp;
    if (pixels.length < stride * height)
        throw new Error('像素缓冲不足');
 ...
 */
export function stitchPng(tiles, totalWidth, totalHeight) {
    if (tiles.length === 0)
        throw new Error('没有可拼接的片段');
    if (totalWidth <= 0 || totalHeight <= 0)
        throw new Error('拼接目标尺寸非法');
    let bpp = 0;
    const decoded = tiles.map((tile) => {
        const d = decodePng(tile.png);
        if (d.bpp !== 3 && d.bpp !== 4)
            throw new Error(`不支持的片段色型: ${d.bpp}`);
        if (bpp === 0)
            bpp = d.bpp;
        else if (d.bpp !== bpp)
            throw new Error('片段颜色类型不一致');
