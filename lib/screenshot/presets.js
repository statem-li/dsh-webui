...
/** 画幅档位顺序（UI 分段选择按此渲染）。 */
export const SHOT_ASPECTS = ['auto', '16:9', '4:3', '1:1', '9:16', '3:4'];
/**
 * 解析画幅参数。
 * @returns 固定比例的宽高比数值；自适应 / 未知值返回 null。
 */
export function shotAspectRatio(aspect) {
 ...
 * @returns 该档的渲染参数。
 */
export function shotPreset(device, quality) {
    const d = device === 'phone' ? 'phone' : 'desktop';
    const q = quality === '1080p' || quality === '4k' ? quality : '2k';
