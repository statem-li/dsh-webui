...
export function digestSummary(text) {
    ...
    if (pieces.length === 0)
        return clampSpeech(body, SUMMARY_TARGET_CHARS);
    let out = '';
    for (const piece of pieces) {
        if (out !== '' && out.length + piece.length > SUMMARY_TARGET_CHARS)
            break;
        out += piece;
        if (out.length >= SUMMARY_TARGET_CHARS)
            break;
    }
            ...
            ctx.webServer.register({
                ...
                handler: async (req, res) => {
                    ...
                    try {
                        const body = await readBody(req);
                        const config = readConfig();
                        if (!config.enabled || !config.summary) {
                            json(res, 200, { ok: true, skipped: 'summary-off' });
                            return;
                        }
