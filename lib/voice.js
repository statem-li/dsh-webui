...
const ConfigSchema = z.object({
    ...
    engine: z.union(['system', 'model']).default('system'),
    ...
    summaryStyle: z.union(['digest', 'llm']).default('digest'),
});
...
function providerConfig(ctx, providerId) {
    try {
        ...
        const entry = entries.find((item) => item.provider === providerId);
 ...
 * @returns 一句话总结（可能为空串）。
 */
export function digestSummary(text) {
 ...
 */
export function applyVoice(ctx) {
    let scope;
    try {
     ...
     * @returns 放行时返回 true。
     */
    function claimFloor(sessionId, kind) {
        if (sessionId === '' || kind === 'test')
            return true;
        if (ownerFree()) {
            owner = sessionId;
    ...
    /** 同文本去重（多标签页会把同一句上报多次）。 */
    function seenRecently(sessionId, text) {
        const now = Date.now();
    ...
    function ensureWorker() {
        ...
        child.stderr.on('data', (chunk) => {
            ...
            if (text !== '')
                console.error('[webui-voice] speaker stderr:', text.slice(0, 300));
        });
        child.on('exit', () => {
            if (worker === child) {
                worker = null;
    ...
    function onWorkerLine(line) {
        ...
        if (line.startsWith('ERR '))
            console.warn('[webui-voice] speaker:', line.slice(4, 300));
    ...
    function writeCommand(child, payload) {
        try {
    ...
    function enqueue(kind, text, sessionId = '') {
        ...
        if (body === '')
            return false;
        if (!claimFloor(sessionId, kind))
            return false;
        if (seenRecently(sessionId, body))
            return false;
     ...
     */
    function stopSpeaking(sessionId = '') {
        if (sessionId === '') {
            queue.length = 0;
            owner = null;
            speakingSession = '';
    ...
    async function synthWithModel(item, config) {
        ...
        try {
            ...
            if (bytes.length === 0)
                return null;
            if (tempDir === null)
                tempDir = mkdtempSync(join(tmpdir(), 'dsh-voice-'));
     ...
     */
    async function llmSummary(text, config) {
        const parts = splitKey(config.modelKey);
        const llm = ctx.get('llm');
        if (parts === null || llm === undefined)
            return '';
        try {
            let out = '';
    ...
    let voicesPending = null;
    function listVoices() {
        if (voicesCache !== null)
            return Promise.resolve(voicesCache);
        if (voicesPending !== null)
            return voicesPending;
        voicesPending = new Promise((resolve) => {
            let out = '';
            let child;
            try {
                child = spawn('powershell.exe', [
    ...
    async function listSpeechModels() {
        try {
            ...
            for (const [key, value] of Object.entries(caps)) {
                if (Array.isArray(value) && value.includes('speech') && splitKey(key) !== null)
                    out.push(key);
            }
            return out.sort();
        }
    ...
    const sessionOf = (body) => typeof body?.sessionId === 'string' ? body.sessionId : '';
    ctx.effect(() => {
        const disposers = [
                ...
                handler: async (req, res) => {
                    ...
                    try {
                        if (req.method === 'POST' && scope !== undefined) {
                            ...
                            for (const key of ['enabled', 'live', 'summary']) {
                                if (typeof body?.[key] === 'boolean')
                                    patch[key] = body[key];
                            }
                            if (body?.engine === 'system' || body?.engine === 'model')
                                patch.engine = body.engine;
                            if (body?.summaryStyle === 'digest' || body?.summaryStyle === 'llm')
                                patch.summaryStyle = body.summaryStyle;
                            for (const key of ['systemVoice', 'modelKey', 'modelVoice']) {
                                if (typeof body?.[key] === 'string')
                                    patch[key] = body[key];
                            }
                            if (Number.isFinite(Number(body?.rate)))
                                patch.rate = Math.min(10, Math.max(-10, Math.round(Number(body.rate))));
                            if (Number.isFinite(Number(body?.volume)))
                                patch.volume = Math.min(100, Math.max(0, Math.round(Number(body.volume))));
            ...
            ctx.webServer.register({
                ...
                handler: async (req, res) => {
                    ...
                    try {
                        ...
                        if (muted) {
                            json(res, 200, { ok: true, skipped: 'muted' });
                            return;
                        }
                        // 试听不看总开关（用户正在设置页调音色）；实时/总结受开关约束。
                        if (kind !== 'test' && !force) {
                            if (!config.enabled) {
                                json(res, 200, { ok: true, skipped: 'disabled' });
                                return;
                            }
                            if (kind === 'live' && !config.live) {
                                json(res, 200, { ok: true, skipped: 'live-off' });
                                return;
                            }
                            if (kind === 'summary' && !config.summary) {
                                json(res, 200, { ok: true, skipped: 'summary-off' });
                                return;
                            }
                        }
            ...
            ctx.webServer.register({
                ...
                handler: async (req, res) => {
                    ...
                    try {
                        ...
                        if (!force && (!config.enabled || !config.summary)) {
                            json(res, 200, { ok: true, skipped: 'summary-off' });
                            return;
                        }
