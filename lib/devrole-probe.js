...
async function probeRole(baseURL, apiKey, model, role) {
 ...
 * 私有网关。
 */
function autoUsesSystem(providerKey, baseURL) {
    if (providerKey === 'opencode' || baseURL.includes('opencode.ai'))
        return true;
    if (baseURL.includes('sensenova.cn'))
        return true;
    ...
    try {
        ...
        if (url.protocol === 'http:')
            return true;
        if (/^(localhost|\d{1,3}(\.\d{1,3}){3})$/i.test(url.hostname))
            return true;
    }
    catch { /* 非法 URL 按未豁免处理 */ }
    return false;
...
async function runProbeAsync(ctx) {
    const st = probe;
    const finish = () => {
        st.running = false;
...
/** 注册 HTTP 接口。 */
export function applyDevRoleProbe(ctx) {
    ctx.effect(() => {
...
//# sourceMappingURL=devrole-probe.js.map