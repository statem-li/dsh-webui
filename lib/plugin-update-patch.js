...
export function parseTreeList(text) {
    const out = new Map();
...
export function parseTreeApi(json) {
    const out = new Map();
 ...
 */
export function splitPatches(diff) {
    const out = [];
    let cur = null;
...
export function applyFilePatch(original, patch) {
    ...
    while (i < patch.lines.length) {
        ...
        if (target < cursor || target > src.length)
            return null;
 ...
 */
export function isInstalledPath(path, files) {
...
//# sourceMappingURL=plugin-update-patch.js.map