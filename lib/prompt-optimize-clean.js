...
function longestFence(text) {
    ...
    if (trimmed === '')
        return null;
    if (trimmed.length * 4 < text.trim().length)
        return null;
    return trimmed;
}
...
function stripTrailingMeta(text) {
    ...
    while (end > 1 && TRAILING_META_RE.test(blocks[end - 1] ?? ''))
        end -= 1;
 ...
 */
function stripUnbalancedFence(text) {
    const lines = text.split('\n');
    const fences = [];
    lines.forEach((line, index) => { if (FENCE_LINE_RE.test(line))
        fences.push(index); });
    if (fences.length === 0 || fences.length % 2 === 0)
        return text;
    if (fences[0] === 0)
        return lines.slice(1).join('\n').trim();
    if (fences[fences.length - 1] === lines.length - 1)
 ...
 */
export function previewOptimized(raw) {
    const base = typeof raw === 'string' ? raw.trim() : '';
    if (base === '')
        return '';
...
//# sourceMappingURL=prompt-optimize-clean.js.map