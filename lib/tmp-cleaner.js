 ...
 */
import { existsSync, mkdirSync, readdirSync, readFileSync, appendFileSync, statSync, rmSync, writeFileSync } from 'node:fs';
...
function normalizeConfig(raw) {
    ...
    return {
        ...
        extraPatterns: Array.isArray(src.extraPatterns)
            ? src.extraPatterns.filter((p) => typeof p === 'string' && p.trim().length > 0).map(p => p.trim()).slice(0, 64)
            : [],
...
const CONVENTION_MEMORY_TEXT = '【工作区临时文件约定】凡是为完成当前任务而临时创建的一次性脚本、探针、数据抓取结果等中间产物，一律写入当前工作区根下的 _tmp/ 目录（不存在就先创建它），不要散落在工作区其他位置；这些文件无需手动删除，dsh-webui 清理器会定期自动清理各工作区的 _tmp/。正式的项目代码与文档不要放进 _tmp/。';
/** 注入的系统提示词片段（fallback：记忆引擎未启用时才走这条通道）。 */
const TMP_CONVENTION_INSTRUCTION = '【工作区临时文件约定】凡是为完成当前任务而临时创建的一次性脚本、探针、数据抓取结果等中间产物，一律写入当前工作区根下的 _tmp/ 目录（不存在就先创建它），不要散落在工作区其他位置；这些文件无需你手动删除，系统会定期自动清理。正式的项目代码与文档不要放进 _tmp/。';
/** 取 memory 模块共享的单例 store（未启用 memory 模块时为 undefined）。 */
function memoryStoreOf(_ctx) {
 ...
 */
async function ensureConventionMemory(ctx) {
    const store = memoryStoreOf(ctx);
    if (store === undefined)
        return false;
    const config = readConfig(ctx);
    const entries = await store.readEntries();
    const managed = entries.filter(e => Array.isArray(e.tags) && e.tags.includes(CONVENTION_TAG));
    try {
        ...
        if (kept !== undefined) {
            ...
            for (const extra of managed.slice(1))
                await store.removeEntry(extra.id);
            return true;
        }
        // 首次创建：写入默认文案的置顶记忆。
...
function activeFileRegexes(config) {
 ...
 */
function collectForWorkspace(root, state) {
    const items = [];
    const emptyDirs = [];
    let count = 0;
    let truncated = false;
    ...
    const walk = (dir) => {
        ...
        for (const name of entries) {
            ...
            if (st.isDirectory()) {
                if (PROTECTED_DIRS.has(name.toLowerCase()))
                    continue;
                if (dir === root && name === TMP_DIR_NAME)
                    continue; // 已单独处理
                walk(full);
 ...
 * roots 缺省用全部已注册工作区。
 */
async function runClean(ctx, options = {}) {
    const config = readConfig(ctx);
...
/** 读取最近 N 条日志（新→旧）。 */
function recentLogs(limit = RECENT_LOG_LIMIT) {
    try {
    ...
    try {
        ...
        for (let i = lines.length - 1; i >= 0; i -= 1) {
            try {
                ...
                if (rec.ts !== undefined && rec.dryRun !== true)
                    return Date.parse(rec.ts);
            }
            catch { /* 跳过坏行 */ }
        }
    }
...
// ── 配置读写 + 调度器 ───────────────────────────────────────────────────────
let settingsScope;
function readConfig(_ctx) {
    if (settingsScope !== undefined) {
        try {
            return normalizeConfig(settingsScope.get());
        }
        catch { /* fallthrough */ }
    }
 ...
 * @param ctx - host 上下文。
 */
export async function applyTmpCleaner(ctx) {
            ...
            try {
                if (req.method === 'POST') {
                    ...
                    if (!body || typeof body.action !== 'string')
                        return respond(400, { ok: false, error: '缺少 action' });
                    if (body.action === 'config') {
                        ...
                        if (typeof body.patch?.injectPrompt === 'boolean') {
                    ...
                    if (body.action === 'run' || body.action === 'preview') {
                        ...
                        const roots = Array.isArray(body.roots) ? body.roots.filter((r) => typeof r === 'string') : undefined;
                ...
                if (config.enabled) {
                    ...
                    if (config.scheduleKind === 'interval') {
                        ...
                        if (scheduled !== null) {
                            ...
                            nextDue = new Date(!ranAfterScheduledToday && Date.now() < scheduled ? scheduled : scheduled + 86_400_000).toISOString();
                        }
                    }
                }
