...
function safeUrl(value, context) {
    ...
    try {
        ...
        if (protocol === 'http:' || protocol === 'https:')
            return url;
        if (protocol === 'mailto:' && context === 'href')
            return url;
        // data:image 位图仅允许出现在 src（支持模型输出的 base64 图）。
        if (context === 'src' && /^data:image\/(?:png|gif|jpe?g|webp|avif|bmp);/i.test(url))
            return url;
        return '';
    }
    catch {
...
function cleanAttr(name, value, tagName) {
    const lower = name.toLowerCase();
    if (lower === '' || !/^[a-zA-Z_:][\w:.-]*$/.test(name))
        return null;
    if (/^on/i.test(lower))
        return null;
    if (lower === 'srcdoc' || lower === 'ping' || lower === 'innerhtml'
        || lower === 'outerhtml' || lower === 'textcontent' || lower === 'innertext')
        return null;
    if (lower === 'style') {
    ...
    if (lower === 'action' || lower === 'formaction' || lower === 'srcset')
        return null;
    if (lower === 'src' || lower === 'poster') {
        const url = safeUrl(value, 'src');
        return url === '' ? null : ` ${name}="${escapeAttr(url)}"`;
    }
    if (lower === 'target' && tagName === 'a')
        return ` ${name}="_blank"`;
    if (lower === 'rel' && tagName === 'a')
        return ` ${name}="noopener noreferrer"`;
    if (lower === 'referrerpolicy' && tagName === 'img')
...
export function sanitizeHtmlFragment(html) {
    let out = '';
    let pos = 0;
    const dropStack = []; // 整块剔除中的危险标签栈（栈顶即最内层）
    const softStack = []; // 软删成对标签（起止皆吞、内容保留）
    const dropping = () => dropStack.length > 0;
    while (pos < html.length) {
        ...
        if (dropping()) {
            if (isClosing && tagName === dropStack[dropStack.length - 1])
                dropStack.pop();
            else if (!isClosing && HARDENED_PAIR_TAGS.has(tagName))
                dropStack.push(tagName);
            continue;
        }
        // 普通树内遇到危险标签：入栈并整块丢弃。
        if (HARDENED_PAIR_TAGS.has(tagName)) {
            dropStack.push(tagName);
            continue;
        }
        ...
        if (SOFT_STRIP_TAGS.has(tagName)) {
            ...
            if (isClosing) {
                if (softStack[softStack.length - 1] === tagName)
                    softStack.pop();
                continue;
            }
            if (selfClosing)
        ...
        if (tagName === 'input') {
            ...
            if (!/type\s*=\s*["']?checkbox/i.test(raw) || !/disabled/i.test(raw))
                continue;
