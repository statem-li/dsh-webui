...
async function subDirsOf(parent) {
    try {
...
/** 安装全部随包技能，返回逐技能结果。 */
async function installAll(force) {
...
/** 把安装结果渲染成人类可读清单。 */
function renderOutcomes(outcomes, targetRoot, force) {
    const lines = [`PlanWeave 技能安装（目标：${targetRoot}${force ? '，force 覆盖' : ''}）：`];
    for (const o of outcomes) {
        if (o.status === 'installed')
            lines.push(`  ✔ ${o.name}：已安装`);
        else if (o.status === 'same')
            lines.push(`  ● ${o.name}：已是最新，跳过`);
        else if (o.status === 'differs')
            lines.push(`  ○ ${o.name}：${o.detail ?? '内容不同'}`);
        else
            lines.push(`  ✖ ${o.name}：失败${o.detail !== undefined ? ` — ${o.detail}` : ''}`);
    }
 ...
 */
export function registerPlanweaveSkillsTool(ctx) {
    return ctx.tools.register(defineTool({
        ...
        presentCall: () => ({ card: 'generic', kind: 'other', title: '安装 PlanWeave 技能', rawInput: null }),
    }));
}
