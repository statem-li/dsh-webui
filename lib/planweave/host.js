...
import { removeTaskNode, addTaskNode, setTaskDependencies, addBlock, removeBlock, updateBlockPrompt, updateBlockPlanning, updateTaskPrompt, updateTaskAcceptance, updateTaskTitle, updateTaskExecutor, builtinExecutorNames, loadPackage, searchProject, getStatistics, getTodoGroups, validateGraphQuality, undoDesktopPlanGraphCommand, redoDesktopPlanGraphCommand, listProjects, initManagedProject, } from '@planweave-ai/runtime';
...
export async function executeClaimStep(engine, env, label, claim) {
    if (claim.kind === 'block') {
...
async function runCoordination(engine, env, maxSteps) {
    ...
    for (let i = 0; i < maxSteps; i += 1) {
        ...
        if (claim.kind !== 'block' && claim.kind !== 'feedback')
            continue;
...
async function handleGraph(ctx, req, res) {
    if (!loopbackAllowed(req)) {
        json(res, 403, { ok: false, error: 'loopback-only' });
        return;
    }
...
function parseRecordsLimit(raw) {
    if (raw === null)
...
function jsonTextField(text, field) {
    try {
 ...
 */
async function collectArtifactHits(resultsDir) {
    const hits = [];
    const add = async (kind, id, ref, taskId, dir) => {
        try {
...
async function collectRecentRecords(resultsDir, limit) {
...
async function handleRecords(ctx, req, res) {
    if (!loopbackAllowed(req)) {
        json(res, 403, { ok: false, error: 'loopback-only' });
        return;
    }
 ...
 */
async function handleSeed(ctx, req, res) {
    if (!loopbackAllowed(req)) {
        json(res, 403, { ok: false, error: 'loopback-only' });
        return;
    }
 ...
 */
async function handleRemoveTask(ctx, req, res) {
    if (!loopbackAllowed(req)) {
        json(res, 403, { ok: false, error: 'loopback-only' });
        return;
    }
 ...
 */
async function handleCreateTask(ctx, req, res) {
    if (!loopbackAllowed(req)) {
        json(res, 403, { ok: false, error: 'loopback-only' });
        return;
    }
 ...
 */
async function handleSetDeps(ctx, req, res) {
    if (!loopbackAllowed(req)) {
        json(res, 403, { ok: false, error: 'loopback-only' });
        return;
    }
...
async function handleAutoRunStart(ctx, req, res) {
    if (!loopbackAllowed(req)) {
        json(res, 403, { ok: false, error: 'loopback-only' });
        return;
    }
    try {
        ...
        const { llm, model } = resolveLlm(ctx);
        const engine = await PlanweaveEngine.open(name);
        const env = { ctx, exec: null, llm, model, provider: defaultSubagentProvider(ctx) };
...
async function handleAutoRunControl(ctx, req, res) {
    if (!loopbackAllowed(req)) {
        json(res, 403, { ok: false, error: 'loopback-only' });
        return;
    }
    try {
        ...
        if (body.action === 'pause')
            snapshot = pauseAutoRunBg(id);
        else if (body.action === 'resume')
            snapshot = resumeAutoRunBg(id);
        else if (body.action === 'stop')
            snapshot = stopAutoRunBg(id);
        else
            throw new Error(`未知操作：${String(body.action)}`);
        if (snapshot === undefined)
            throw new Error('Auto Run 不存在');
        json(res, 200, { ok: true, snapshot });
    }
    catch (error) {
        json(res, 500, { ok: false, error: error instanceof Error ? error.message : String(error) });
...
async function handleAutoRunState(ctx, req, res) {
    if (!loopbackAllowed(req)) {
        json(res, 403, { ok: false, error: 'loopback-only' });
        return;
    }
 ...
 */
async function handleRecordContent(ctx, req, res) {
    if (!loopbackAllowed(req)) {
        json(res, 403, { ok: false, error: 'loopback-only' });
        return;
    }
...
async function handleReveal(ctx, req, res) {
    ...
    try {
        ...
        if (process.platform === 'win32')
            ...
            spawn('open', [resolvedDir]);
        else
            spawn('xdg-open', [resolvedDir]);
 ...
 */
async function handleEdit(ctx, req, res) {
    if (!loopbackAllowed(req)) {
        json(res, 403, { ok: false, error: 'loopback-only' });
        return;
    }
    ...
    try {
        ...
        switch (op) {
            case 'block.add': {
                const type = body.type === 'review' ? 'review' : 'implementation';
                const title = str(body.title).trim();
                if (title === '')
                    throw new Error('块标题不能为空');
            ...
            case 'block.planning': {
                ...
                if (typeof body.reviewRequired === 'boolean')
                    fields.reviewRequired = body.reviewRequired;
                if (Number.isFinite(Number(body.maxFeedbackCycles)))
                    fields.maxFeedbackCycles = Math.max(0, Math.round(Number(body.maxFeedbackCycles)));
...
async function handleTaskSource(ctx, req, res) {
    if (!loopbackAllowed(req)) {
        json(res, 403, { ok: false, error: 'loopback-only' });
        return;
    }
...
async function handleExecutors(ctx, req, res) {
    if (!loopbackAllowed(req)) {
        json(res, 403, { ok: false, error: 'loopback-only' });
        return;
    }
...
async function handleListProjects(ctx, req, res) {
    if (!loopbackAllowed(req)) {
        json(res, 403, { ok: false, error: 'loopback-only' });
        return;
    }
...
async function handleStatistics(ctx, req, res) {
    if (!loopbackAllowed(req)) {
        json(res, 403, { ok: false, error: 'loopback-only' });
        return;
    }
...
/** GET /todos：待办分组透传。 */
async function handleTodos(ctx, req, res) {
    if (!loopbackAllowed(req)) {
        json(res, 403, { ok: false, error: 'loopback-only' });
        return;
    }
...
/** GET /quality：图质量校验报告。 */
async function handleQuality(ctx, req, res) {
    if (!loopbackAllowed(req)) {
        json(res, 403, { ok: false, error: 'loopback-only' });
        return;
    }
...
async function handleGraphHistory(ctx, req, res) {
    if (!loopbackAllowed(req)) {
        json(res, 403, { ok: false, error: 'loopback-only' });
        return;
    }
 ...
 */
async function handleRun(ctx, req, res) {
    if (!loopbackAllowed(req)) {
        json(res, 403, { ok: false, error: 'loopback-only' });
        return;
    }
...
export function applyPlanweaveHost(ctx) {
    ...
    try {
        ...
        console.warn('[planweave] settings.register failed — 设置里将看不到 PlanWeave 卡片:', error instanceof Error ? error.message : String(error));
    }
    // 2) 模型工具。
