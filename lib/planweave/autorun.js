...
import { executeClaimStep } from './host.js';
const runs = new Map();
...
function push(internal, line) {
    internal.events.push(`[${nowClock()}] ${line}`);
    if (internal.events.length > MAX_EVENTS)
        internal.events.splice(0, internal.events.length - MAX_EVENTS);
}
function snapshotOf(internal) {
    return {
        id: internal.id,
...
/** 暂停。 */
export function pauseAutoRunBg(id) {
    const internal = getInternal(id);
    if (internal === undefined || internal.status !== 'running')
...
export function resumeAutoRunBg(id) {
    const internal = getInternal(id);
    if (internal === undefined || internal.status !== 'paused')
        return internal === undefined ? undefined : snapshotOf(internal);
...
/** 停止（终态；进行中的一步完成后退出）。 */
export function stopAutoRunBg(id) {
    const internal = getInternal(id);
...
/** 查询快照。 */
export function getAutoRunBgState(id) {
    const internal = getInternal(id);
    return internal === undefined ? undefined : snapshotOf(internal);
}
/** 最近一次启动（按项目名过滤；找不到返回 undefined）。 */
export function latestAutoRunBg(projectName) {
    let found;
    for (const internal of runs.values()) {
        if (internal.projectName !== projectName)
            continue;
        if (found === undefined || internal.startedAt > found.startedAt)
            found = internal;
    }
