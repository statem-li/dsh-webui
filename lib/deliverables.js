...
function isExcludedPath(absPath) {
    ...
    if (EXCLUDED_FRAGMENTS.some(fragment => norm.includes(fragment)))
        return true;
    const name = norm.slice(norm.lastIndexOf('/') + 1);
    return EXCLUDED_NAMES.has(name);
}
function rewriteLog(sessionId, map) {
...
function recordWrite(absPath, sessionId) {
    ...
    // Math.max 合并，天然幂等。
    if ((map.get(absPath) ?? Number.NEGATIVE_INFINITY) >= Date.now())
        return;
    const now = Date.now();
    map.set(absPath, now);
    try {
...
async function resolveRecorded(ctx, sessionId, rawPath) {
    ...
    const fs = ctx.get('fs');
    if (fs === undefined)
        throw new HttpError(500, 'fs service unavailable');
    try {
...
async function readContent(ctx, sessionId, rawPath) {
...
async function readBin(ctx, sessionId, rawPath) {
    const target = await resolveRecorded(ctx, sessionId, rawPath);
    const fs = ctx.get('fs');
    const info = await fs.stat(target);
    if (info === undefined)
...
async function readRaw(ctx, sessionId, rawPath) {
    const target = await resolveRecorded(ctx, sessionId, rawPath);
    const fs = ctx.get('fs');
    const info = await fs.stat(target);
    if (info === undefined)
...
async function handle(ctx, req, res) {
    try {
        ...
        if (error instanceof Error && typeof error.code === 'string') {
            json(res, fsErrorStatus(error), { ok: false, error: error.message, code: error.code });
            return;
        }
 ...
 */
export function applyDeliverables(ctx) {
    ...
    ctx.effect(() => webServer.register({
        kind: 'prefix',
        path: ROUTE_PREFIX,
        handler: (req, res) => { void handle(ctx, req, res).catch(() => { }); },
