...
const sessionWrittenTimes = new Map();
/** 已从磁盘加载过写入日志的会话（避免重复读文件）。 */
const writtenLogLoaded = new Set();
 ...
 * 返回 null 表示无法表示（非法路径）。
 */
function resolveSnapshotKey(cwd, rawPath) {
...
/** 快照索引落盘路径。 */
function snapshotMetaPath(sessionId, seq) {
...
function ensureDirSync(dir) {
    ...
    catch { /* 失败留给 append 的 try/catch */ }
    ensuredDirs.add(dir);
}
/** 追加一行到 JSONL 日志；失败静默（记录丢失只影响退回范围，不影响快照）。 */
function appendWrittenLine(file, line) {
    try {
 ...
 */
export function writtenPathsFor(sessionId) {
 ...
 * 维持升级前的兼容行为。
 */
export function writtenPathsAfter(sessionId, afterMs) {
...
async function writeBlobIfAbsent(hash, plain) {
    if (knownBlobs.has(hash))
        return false;
...
async function entryBuffer(entry) {
    if (entry.hash !== null) {
 ...
 */
let cachedExternalDirs = null;
export function externalDirs() {
    if (cachedExternalDirs !== null)
        return cachedExternalDirs;
    const candidates = [join(homedir(), 'Desktop')];
    const oneDrive = process.env.OneDrive ?? process.env.ONEDRIVE;
    if (typeof oneDrive === 'string' && oneDrive !== '')
        candidates.push(join(oneDrive, 'Desktop'));
    const seen = new Set();
...
export function captureSnapshotSync(cwd, extraPaths) {
    ...
    let stopped = false;
    // 记录一个「只存存在性」的文件。
    const recordUnavailable = (key, size, flag) => {
        files[key] = { size, flag };
    };
...
export function buildSnapshotV2(sessionId, seq, cwd, captured) {
    const files = {};
...
export async function persistSnapshot(args) {
    ...
    for (const [key, f] of Object.entries(captured.files)) {
        if (f.hash === undefined || captured.buffers.has(f.hash))
            continue;
        if (knownBlobs.has(f.hash) || existsSync(blobPath(f.hash)))
            continue;
...
/** 删除一个快照及其 meta 索引。 */
async function removeSnapshotFiles(sessionId, seq) {
    try {
        await rm(snapshotPath(sessionId, seq), { force: true });
    }
    catch { /* 忽略 */ }
    try {
        await rm(snapshotMetaPath(sessionId, seq), { force: true });
    }
    catch { /* 忽略 */ }
}
...
async function peekJsonNumber(file, field) {
    let fh;
    try {
...
async function collectAndAgeOut(now) {
    const records = [];
...
export async function gcBlobs() {
    ...
    // 即使对应索引尚未落盘也不可回收。
    for (const cached of hashCache.values())
        live.add(cached.hash);
    const cutoff = Date.now() - BLOB_ORPHAN_GRACE_MS;
    const { paths } = await collectBlobs();
    let removed = 0;
    for (const blob of paths) {
        if (live.has(blob.hash))
            continue;
        if (blob.mtimeMs > cutoff)
            continue; // 宽限期内的不视为孤儿
        try {
...
export async function runMaintenance() {
    ...
    for (const rec of ageOut.records)
        total += rec.bytes;
...
function scheduleMaintenance() {
    const now = Date.now();
    if (maintenanceRunning || now - lastMaintenanceAt < MAINTENANCE_INTERVAL_MS)
        return;
...
function parseSnapshot(raw) {
    ...
    const obj = parsed;
    if (typeof obj.cwd !== 'string' || typeof obj.files !== 'object' || obj.files === null)
        return null;
    if (obj.version === 2) {
        return viewOfV2(parsed);
    }
    if (obj.version === 1) {
 ...
 */
function collectLineage(ctx, sessionId) {
 ...
 */
export function writtenPathsAfterLineage(ctx, sessionId, afterMs) {
    const times = new Map();
 ...
 */
export async function restoreSnapshot(view, extraPaths, writtenPaths) {
    const cwd = resolve(view.cwd);
    let restored = 0;
    let skippedLarge = 0;
    let deleted = 0;
    /** 把一个条目的内容写回目标路径；不可恢复返回 false。 */
    const writeBack = async (abs, entry) => {
    ...
    for (const [key, entry] of Object.entries(view.entries)) {
        ...
        if (isExcludedFile(key.split('/').pop() ?? ''))
            continue;
    ...
    const walkAndDeleteExternal = async (dir) => {
        if (insideCwd(cwd, dir))
            return; // cwd 内已由 walkAndDelete 处理
        let entries;
        try {
            entries = await readdir(dir, { withFileTypes: true });
        }
    ...
    for (const dir of extRoots)
        await walkAndDeleteExternal(resolve(dir));
 ...
 */
export function validateRewind(ctx, sessionId, seq) {
    const sessions = ctx.get('sessions');
 ...
 */
export function rewindContext(ctx, sessionId, seq, shadowedSeqs) {
    const sessions = ctx.get('sessions');
    const session = sessions?.get(sessionId);
    if (session === undefined) {
        return { ok: false, status: 409, error: `会话 ${sessionId} 未加载（请先打开该会话再退回）` };
    ...
    if (meter !== undefined) {
        ...
        for (const s of shadowedSeqs) {
            ...
            if (msg !== null)
                shadowedTokenCount += meter.estimateMessage(msg);
        }
        try {
...
async function entryDiffers(current, entry) {
    if (entry.content !== null)
        return !current.equals(Buffer.from(entry.content, 'base64'));
    if (entry.hash !== null)
        return createHash('sha1').update(current).digest('hex') !== entry.hash;
    return false;
}
/**
 ...
 */
export async function diffSnapshot(view, extraPaths, writtenPaths) {
    const cwd = resolve(view.cwd);
    const modified = [];
    const added = [];
    const deleted = [];
        ...
        for (const raw of writtenPaths) {
            ...
            if (entry !== undefined) {
                if (!entryAvailable(entry))
                    continue; // 不可恢复：restore 保留现状，不参与
                let current = null;
                try {
                ...
                if (await entryDiffers(current, entry))
                    modified.push(key);
            }
            else {
                // 快照里没有：当前仍存在 → 会被删除（新增）。
    ...
    const walkAddedExternal = async (dir) => {
        if (insideCwd(cwd, dir))
            return; // cwd 内已由 walk 处理
        let entries;
        try {
            entries = await readdir(dir, { withFileTypes: true });
        }
        ...
        for (const entry of entries) {
            ...
            if (isExcludedFile(entry.name))
                continue;
            const key = abs.split(sep).join('/');
            if (snapshotKeys.has(key) || addedSeen.has(key))
                continue;
            try {
                await stat(abs);
                addedSeen.add(key);
    ...
    const extRoots = view.extDirs ?? externalDirs();
    for (const dir of extRoots)
        await walkAddedExternal(resolve(dir));
...
async function entryTextOf(entry) {
    if (entry === undefined)
        return null;
    const buf = await entryBuffer(entry);
...
async function handle(ctx, req, res) {
    ...
    try {
            ...
            try {
                ...
                for (const id of collectLineage(ctx, sessionId)) {
                    for (const seq of await listSnapshotSeqs(id)) {
                        let meta = null;
                        try {
                            meta = JSON.parse(await readFile(snapshotMetaPath(id, seq), 'utf8'));
                            if (meta.version !== 1 || typeof meta.cwd !== 'string' || meta.files === null || typeof meta.files !== 'object')
                                meta = null;
                        }
                        catch {
                            meta = null;
                ...
                for (const point of bySeq.values()) {
                    if (point.hash === prevHash) {
                        ...
                        if (last !== undefined) {
                            last.seq = point.seq;
                            last.createdAt = point.createdAt;
                            last.size = point.size;
                        }
                        continue;
                    }
                    prevHash = point.hash;
        ...
        if (method === 'GET' && rest === '/compare') {
            ...
            const left = { text: leftText, unavailable: leftUnavailable, size: leftSize };
            let right;
            if (seqBRaw !== null) {
        ...
        if (method === 'POST' && rest === '/rewind') {
            ...
            await pruneFromSeq(sessionId, seqRaw).catch(() => { });
